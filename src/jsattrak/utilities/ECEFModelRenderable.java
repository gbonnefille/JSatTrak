...
public class ECEFModelRenderable implements Renderable {
	...
	SphereObject sphere = new SphereObject(new Vec4(0, 0, 0, 0), sphereRadius,
			true);

	SphereObject eventSphere = new SphereObject(new Vec4(0, 0, 0, 0),
			sphereRadius, true);
	...
	public void render(DrawContext dc) {
		...
		// gl.glEnable(GL.GL_TEXTURE_2D); // removed so the sun shading wouldn't
		// effect line colors
		gl.glPushAttrib(javax.media.opengl.GL.GL_TEXTURE_BIT
														...
														// nodes
		{
			...
			// GROUND TRACK
			if (sat.getSatOptions().isShowGroundTrack3d()) {
				...
				for (int i = 0; i < sat.getNumGroundTrackLagPts(); i++) {
					...
					gl.glVertex3f((float) ptLoc.x, (float) ptLoc.y,
							(float) ptLoc.z);
			...
			// ECEF ORBIT TRACE
			if (sat.getSatOptions().isShow3DOrbitTrace()
					&& !sat.getSatOptions().isShow3DOrbitTraceECI()) {
				Vec4 ptLoc;
				// ground trace - lag
				gl.glBegin(GL.GL_LINE_STRIP); // GL_LINE_STRIP
				for (int i = 0; i < sat.getNumGroundTrackLagPts(); i++) {
				...
				while (eventPositionIterator.hasNext()) {
					...
					Vec4 pos = globe.computePointFromPosition(
							Angle.fromDegrees(eventPosition[1] * 180.0
									/ Math.PI),
							Angle.fromDegrees(eventPosition[2] * 180.0
									/ Math.PI), eventPosition[3]);

					eventSphere.setCenter(pos);

					eventSphere.setColor(sat.getSatOptions().getSatColor());
					// sphere.setCenter(-xyz[0], xyz[2], xyz[1]);
					eventSphere.render(dc);

					AnnotationAttributes geoAttr = createFontAttribs(sat
							.getSatOptions().getSatColor());
					GlobeAnnotation an = new GlobeAnnotation(eventName,
							Position.fromDegrees(eventPosition[1] * 180.0
									/ Math.PI, eventPosition[2] * 180.0
									/ Math.PI, eventPosition[3]), geoAttr);
					// annotation - without any attribs, gives a bubble box
					// annotation doesn't strech well in GLCanvas
					// GlobeAnnotation an = new
					// GlobeAnnotation(gs.getStationName(),
					// Position.fromDegrees(gs.getLatitude(), gs.getLongitude(),
					// gs.getAltitude()));
					an.render(dc);
		...
		for (GroundStation gs : gsHash.values()) // search through all sat nodes
		{
			if (gs.isShow3D()) {
				...
				Vec4 pos = globe.computePointFromPosition(
						Angle.fromDegrees(gs.getLatitude()),
						Angle.fromDegrees(gs.getLongitude()), gs.getAltitude());

				sphere.setCenter(pos);
				// sphere.setCenter(-xyz[0], xyz[2], xyz[1]);
				sphere.render(dc);

				// create name --- Hmm this creates a lot of annotations every
				// time step, is this a problem?
				// doesn't work to render and change attributes, maybe save one
				// per GroundStation?
				if (gs.isShow3DName()) {
					AnnotationAttributes geoAttr = createFontAttribs(gs
							.getStationColor());
					GlobeAnnotation an = new GlobeAnnotation(
							gs.getStationName(), Position.fromDegrees(
									gs.getLatitude(), gs.getLongitude(),
									gs.getAltitude()), geoAttr);
					// annotation - without any attribs, gives a bubble box
					// annotation doesn't strech well in GLCanvas
					// GlobeAnnotation an = new
					// GlobeAnnotation(gs.getStationName(),
	...
	private AnnotationAttributes createFontAttribs(Color textColor) {
		...
		geoAttr.setEffect(MultiLineTextRenderer.EFFECT_OUTLINE); // Black
																	// outline
