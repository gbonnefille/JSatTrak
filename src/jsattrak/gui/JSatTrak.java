...
public class JSatTrak extends javax.swing.JFrame implements
	...
	private String versionString = "Version 4.1  (10 July 2009)"; // Version of
																	// app
	...
	private boolean epochTimeEqualsCurrentTime = true; // uses current time for
														...
														// button)
	private Time scenarioEpochDate = new Time(); // scenario epoch if
	...
	public JSatTrak() {
		...
		try {
			...
			// for (J2DEarthPanel w : twoDWindowVec) //strange hack to make the
			// map repaint correctly with the LAF (needed if done at the end)
			// {
		...
		currentJulianDate.update2CurrentTime(); // update();// =
		...
		currentJulianDate.add(Time.MILLISECOND, 1000 - mil); // remove the
																// milliseconds
																// (so it shows
																// an even
																// second)
		...
		try {
			...
			if ((new File(tleDownloader.getLocalPath()).exists())
				...
				satHash.get("ISS (ZARYA)             ").setThreeDModelPath(
						"isscomplete/iss_complete.3ds");// set default 3d model
		...
		if (false) {
			coverageAnalyzer = new CoverageAnalyzer(currentJulianDate); // start
																		// after
																		// first
																		// step
	...
	{// GEN-HEADEREND:event_resetTimeButtonActionPerformed
		...
		if (epochTimeEqualsCurrentTime) {
			...
			// update to epoch
			currentJulianDate.set(scenarioEpochDate
					.getCurrentGregorianCalendar().getTimeInMillis());
		}
	...
	private JInternalFrame createNew3DInternalWindow() {
		...
		J3DEarthInternalPanel newPanel = new J3DEarthInternalPanel(iframe,
				satHash, gsHash, currentJulianDate.getMJD(), this);
	...
	public void updateTime(boolean eventDetector) {
		...
		if (realTimeModeCheckBox.isSelected()) {
			...
			currentJulianDate.addSeconds(currentPlayDirection
		...
		// System.out.println("Sun Pos(J2K) - Date (MJD): " +
		// currentJulianDate.getMJD() + ", <x,y,z> < " +sunPos[0] +", "
		// +sunPos[1] +", "+sunPos[2] +" >");
		// sunPos = sun.getCurrentPositionMOD();
		// System.out.println("Sun Pos(MOD) - Date (MJD): " +
		// currentJulianDate.getMJD() + ", <x,y,z> < " +sunPos[0] +", "
		// +sunPos[1] +", "+sunPos[2] +" >");
		// double[] llaTemp = GeoFunctions.GeodeticJulDate(
		// sun.getCurrentPositionMOD() ,currentJulianDate.getJulianDate());
		// // sun.getCurrentPositionMOD()
		// //double[] llaTemp = GeoFunctions.GeodeticJulDate( new double[]
		// {-1.400954880970050E+08, 5.168955443226393E+07 ,
		// 2.240975286312218E+07} ,currentJulianDate.getJDN());
		// // 01 Sep 2007 00:00:00.000 UTC
		// System.out.println("Sun lat/long :  " + llaTemp[0]*180.0/Math.PI +
		// " , " + llaTemp[1]*180.0/Math.PI);
	...
	private void coverageMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_coverageMenuItemActionPerformed
		...
		if (coverageAnalyzer == null) {
			coverageAnalyzer = new CoverageAnalyzer(currentJulianDate); // setup
																		// new
																		// analyzer
		...
		JCoverageDialog coverageBrowser = new JCoverageDialog(coverageAnalyzer,
				currentJulianDate, this, satHash, twoDWindowVec);
	...
	public JTrackingPanel openTrackingToolSelectorWindow() {
		...
		JTrackingPanel trackingBrowser = new JTrackingPanel(satHash, gsHash,
				currentJulianDate.getDateTimeStr(), currentJulianDate, this);
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				if (obj instanceof JstSaveClass) // it better be
				{
					...
					satHash.clear();
					Hashtable<String, AbstractSatellite> tempHash = openClass
							.getSatHash();
