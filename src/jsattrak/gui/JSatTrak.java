 ...
 */
public class JSatTrak extends javax.swing.JFrame implements
		InternalFrameListener, WindowListener, Serializable {

	private static final long serialVersionUID = 7647465736296922688L;

	private String versionString = "Version 4.1.4  (7 Feb 2013)"; // Version of
																	// app
	...
	// Satellite property vector to store all the windows (so they can be
	// updated)
	...
	// ----- TIME parameters --------------------
	// UTCG
	// JulianDay currentJulianDate = new JulianDay(); // current sim or real
	// time (Julian Date)
//	Time currentJulianDate = new Time(); // current sim or real time (Julian
											// Date)
	...
	TimeOrekit currentOrekitTime = null;

	// animation parameters
	private Timer playTimer;
	private boolean stopHit = false;
	private int realTimeAnimationRefreshRateMs = 1000; // refresh rate for real
														// time animation
	private int nonRealTimeAnimationRefreshRateMs = 50; // refresh rate for
														// non-real time
														// animation
	private int animationRefreshRateMs = nonRealTimeAnimationRefreshRateMs; // (current)Milliseconds
																			...
																			// in
																			// animation
	private double animationSimStepSeconds = 1.0; // dt in Days per animation
													// step/time update
	int currentPlayDirection = 0; // 1= forward, -1=backward, =0-no animation
									// step, but can update time (esentially a
									// graphic ini or refresh)

	// these are listed in seconds
	double[] timeStepSpeeds = new double[] { 0.0001, 0.001, 0.01, 0.1, 0.25,
			0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0, 300.0, 600.0, 1800.0, 3600.0,
			43200.0, 86400.0, 604800.0, 2419200.0, 31556926.0 };
	...
	private boolean epochTimeEqualsCurrentTime = true; // uses current time for
														// scenario epoch (reset
														// button)
	
	
	
	private TimeOrekit scenarioEpochDate = null; // scenario epoch if
													// epochTimeisCurrentTime =
													// false
	...
	// time dependent objects that should be update when time is updated -- NEED
	// TO BE SAVED?
	...
	public JSatTrak() {
		...
		try {
			currentOrekitTime = new TimeOrekit(currentTimeInMillis);
			scenarioEpochDate = new TimeOrekit(currentTimeInMillis);
		...
		// if User has java 6u10 or greater that means NimbusLookAndFeel is
		// supported!
		try {
		...
		// Locale.setDefault(Locale.CANADA_FRENCH); // reproduces TLE parsing
		// error
		// Locale.setDefault(Locale.US); // forces it to work... but maybe not
		// the best way (okay, fixed in SDP4 - set decimal seperator there)
		...
		super.setIconImage(Toolkit.getDefaultToolkit().getImage(
				getClass().getResource("/icons/logo/JSatTrakLogo_32.png")));
		...
		sun = new Sun(currentOrekitTime.getCurrentOrekitTime()
		...
		// // TEST
		// JObjectListInternalFrame t = new
		// JObjectListInternalFrame(satHash,gsHash,this);
		...
		currentOrekitTime.update2CurrentTime();

		// set time string format
		currentOrekitTime.setDateFormat(dateformat);
		...
		// this.setVisible(true); // only needed to debug if there is a problem
		// below this point in constructor
		...
		// JOptionPane.showInternalMessageDialog(mainDesktopPane,
		// "3D Error (Check your JOGL Installation):\n " + e.toString(),
		...
		idleIcon = new javax.swing.ImageIcon(getClass().getResource(
				"/icons/busyicons/idle-icon.png"));

		// busy-icon0.png
		for (int i = 0; i < 15; i++) {
			busyIcons[i] = new javax.swing.ImageIcon(getClass().getResource(
					"/icons/busyicons/busy-icon" + i + ".png"));
		}
		...
		setStatusMessage("Welcome to JSatTrak " + versionString
				+ ", by Shawn E. Gano");
		...
		try {
			...
			// set the scope so it is in this object (no need to use jsattrak.
			// all the time)
			// doesn't work this way?
			// beanShellInterp.eval("setNameSpace(jsattrak.namespace)");

			// set this so closing the bean shell Desktop doesn't close the
			// whole app
			...
			System.out.println("Error saving main object to bean shell:"
					+ e.toString());
		}
		...
		try {
			...
			if ((new File(tleDownloader.getLocalPath()).exists())
				...
				} else // default scene - ISS
				{
					...
					satHash.get("ISS (ZARYA)             ").setThreeDModelPath(
							"isscomplete/iss_complete.3ds");// set default 3d
															// model
				}
			}
		} catch (Exception e) {
		...
		if (false) {
			...
			coverageAnalyzer = new CoverageAnalyzer(currentOrekitTime); // start
																		// after
																		// first
																		// step
			// CoverageAnalyzer coverageAnalyzer = new
			// CoverageAnalyzer(currentJulianDate,timeStepSpeeds[currentTimeStepSpeedIndex]/(24.0*60*60),satHash);
			twoDWindowVec.get(0).addRenderableObject(coverageAnalyzer); // add
																		// it to
																		// the
																		// panel
			coverageAnalyzer
					.addSatToCoverageAnaylsis("ISS (ZARYA)             ");
			// coverageAnalyzer.addSatToCoverageAnaylsis("test");
			timeDependentObjects.add(coverageAnalyzer); // add object to time
														// updates
		...
		// fire resize function - to correct
		// "dissapearing or transparent windows" issues
		int sizeJump = 1;
		this.setSize(this.getSize().width + sizeJump, this.getSize().height
				+ sizeJump);
		this.setSize(this.getSize().width - sizeJump, this.getSize().height
				- sizeJump);
		...
		if (usingNimbus) {
			try {
				UIManager
						.setLookAndFeel("com.sun.java.swing.plaf.nimbus.NimbusLookAndFeel");
				SwingUtilities.updateComponentTreeUI(this); // apply look and
															// feel over current
															// L&F (otherwise
															// nimbus shows up
															// in correctly)
				...
				System.out
						.println("Sorry no Nimbus LookAndFeel needs java 6u10 or higher!");
				// ex.printStackTrace();
			}
		} // if using nimbus
	...
	public void checkAndInstallPlugins() {
		// first clean out all current plugins displayed (in case it is
		// refreshed)
		...
		if (rootFile.exists() && rootFile.isDirectory()) {
			...
			for (String fileName : rootFile.list(new FileTypeFilter("bsh"))) {
				...
				newPluginMenuItem.setText(menuName); // reset name on menu item
														// (since action over
														// writes it)
			}
		...
		AbstractAction refreshPluginAction = new AbstractAction(
	...
	private void initComponents() {
		...
		saveButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/document-save.png"))); // NOI18N
		...
		openSatBrowserButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/custom/sat_icon_tle.png"))); // NOI18N
		openSatBrowserButton.setToolTipText("Open Satellite Browser"); // NOI18N
		openSatBrowserButton
		...
		openGSbrowser.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/custom/groundStation_obj.png"))); // NOI18N
		...
		resetTimeButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/media-seek-backward.png"))); // NOI18N
		...
		playBackButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource(
						"/icons/gnome_2_18/media-playback-playbackwards.png"))); // NOI18N
		...
		stepBackButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/media-skip-backward.png"))); // NOI18N
		...
		stopButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/media-playback-stop.png"))); // NOI18N
		...
		stepForwardTimeButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/media-skip-forward.png"))); // NOI18N
		stepForwardTimeButton.setToolTipText("Step Forwards"); // NOI18N
		stepForwardTimeButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		playButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/media-playback-start.png"))); // NOI18N
		...
		incrementTimeStepButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/view-sort-ascending.png"))); // NOI18N
		incrementTimeStepButton.setToolTipText("Faster"); // NOI18N
		incrementTimeStepButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		decementTimeStepButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/view-sort-descending.png"))); // NOI18N
		decementTimeStepButton.setToolTipText("Slower"); // NOI18N
		decementTimeStepButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		realTimeModeCheckBox.setBorder(javax.swing.BorderFactory
		...
		jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/other/xclock.png"))); // NOI18N
		jLabel2.setToolTipText("Real Time Mode"); // NOI18N
		jToolBar.add(jLabel2);

		localTimeZoneCheckBox.setText("Local TZ"); // NOI18N
		localTimeZoneCheckBox
				.setToolTipText("Display Local Time Zone? (Otherwise UTC)"); // NOI18N
		localTimeZoneCheckBox.setBorder(javax.swing.BorderFactory
				.createEmptyBorder(0, 0, 0, 0));
		localTimeZoneCheckBox
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		zoomIntoggleButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/zoom-in.png"))); // NOI18N
		zoomIntoggleButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		zoomOuttoggleButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/zoom-out.png"))); // NOI18N
		zoomOuttoggleButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		recenterToggleButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/zoom-best-fit.png"))); // NOI18N
		recenterToggleButton.setToolTipText("2D Recenter"); // NOI18N
		recenterToggleButton
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		screenShotButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/applets-screenshooter.png"))); // NOI18N
		...
		createMovieButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/applications-multimedia.png"))); // NOI18N
		...
		toolbar3DWindowButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/emblem-web.png"))); // NOI18N
		toolbar3DWindowButton.setToolTipText("New 3D Window (External)");
		toolbar3DWindowButton.setFocusable(false);
		toolbar3DWindowButton
				.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
		toolbar3DWindowButton
				.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
		toolbar3DWindowButton
		...
		satListInternalFrame.setFrameIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/logo/JSatTrakLogo_16.png"))); // NOI18N
		satListInternalFrame.setVisible(true);

		javax.swing.GroupLayout satListInternalFrameLayout = new javax.swing.GroupLayout(
				satListInternalFrame.getContentPane());
		satListInternalFrame.getContentPane().setLayout(
				satListInternalFrameLayout);
		satListInternalFrameLayout
				.setHorizontalGroup(satListInternalFrameLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGap(0, 174, Short.MAX_VALUE));
		satListInternalFrameLayout.setVerticalGroup(satListInternalFrameLayout
		...
		javax.swing.GroupLayout desktopPanelLayout = new javax.swing.GroupLayout(
				desktopPanel);
		desktopPanel.setLayout(desktopPanelLayout);
		desktopPanelLayout.setHorizontalGroup(desktopPanelLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addComponent(desktopScrollPane,
						javax.swing.GroupLayout.DEFAULT_SIZE, 814,
						Short.MAX_VALUE));
		desktopPanelLayout.setVerticalGroup(desktopPanelLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addComponent(desktopScrollPane,
		...
		statusAnimationLabel.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/busyicons/idle-icon.png"))); // NOI18N
		...
		newMenuItem.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/window-new.png"))); // NOI18N
		...
		openMenuItem.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/folder-open.png"))); // NOI18N
		...
		closeMenuItem.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/window-close.png"))); // NOI18N
		...
		saveAsMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/document-save-as.png"))); // NOI18N
		...
		exitMenuItem.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/process-stop.png"))); // NOI18N
		...
		simPropMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/control-center2.png"))); // NOI18N
		...
		realTimeMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_R,
				java.awt.event.InputEvent.CTRL_MASK));
		realTimeMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/xclock.png"))); // NOI18N
		...
		nonRealTimeMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_R,
		...
		utcTimeMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_U,
				java.awt.event.InputEvent.CTRL_MASK));
		utcTimeMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/appointment-soon.png"))); // NOI18N
		...
		localTimeMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_U,
				java.awt.event.InputEvent.SHIFT_MASK
						| java.awt.event.InputEvent.CTRL_MASK));
		...
		tleLoaderMenuItem
				.setIcon(new javax.swing.ImageIcon(
						getClass()
								.getResource(
										"/icons/tango_blue_materia/network-transmit-receive.png"))); // NOI18N
		...
		trackToolMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_T,
				java.awt.event.InputEvent.CTRL_MASK));
		trackToolMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/network-wireless.png"))); // NOI18N
		...
		coverageMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource(
						"/icons/other/preferences-system-network-proxy.png"))); // NOI18N
		...
		movieWholeAppMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/applications-multimedia.png"))); // NOI18N
		movieWholeAppMenuItem.setText("Create Movie of Application");
		movieWholeAppMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		commandServerMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/tango_blue_materia/network-receive.png"))); // NOI18N
		commandServerMenuItem.setText("Command Server");
		commandServerMenuItem.setToolTipText("TCP/IP command server");
		commandServerMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		showSatBrowser.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_B,
				java.awt.event.InputEvent.CTRL_MASK));
		showSatBrowser.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/custom/folder-open_sat.png"))); // NOI18N
		...
		showGSBrowserMenuItem.setAccelerator(javax.swing.KeyStroke
				.getKeyStroke(java.awt.event.KeyEvent.VK_G,
						java.awt.event.InputEvent.CTRL_MASK));
		showGSBrowserMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/custom/folder-open_gs.png"))); // NOI18N
		showGSBrowserMenuItem.setText("Ground Station Browser"); // NOI18N
		showGSBrowserMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		new2DWindowMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/gnome-mime-image.png"))); // NOI18N
		...
		newInternal3DWindowMenuItem.setIcon(new javax.swing.ImageIcon(
				getClass().getResource("/icons/other/emblem-web.png"))); // NOI18N
		newInternal3DWindowMenuItem.setText("New 3D Window (Internal)"); // NOI18N
		newInternal3DWindowMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		newExternal3DWindowMenuItem.setIcon(new javax.swing.ImageIcon(
				getClass().getResource("/icons/other/emblem-web.png"))); // NOI18N
		newExternal3DWindowMenuItem.setText("New 3D Window (External)"); // NOI18N
		newExternal3DWindowMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		twoDwindowPropMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/preferences-system.png"))); // NOI18N
		twoDwindowPropMenuItem.setText("2D Window Properties"); // NOI18N
		twoDwindowPropMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		consoleMenuItem.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/other/logviewer.png"))); // NOI18N
		...
		commandShellMenuItem.setIcon(new javax.swing.ImageIcon(
				getClass().getResource(
						"/icons/tango_blue_materia/utilities-terminal.png"))); // NOI18N
		commandShellMenuItem.setText("Command Shell");
		commandShellMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
	...
	private void nonRealTimeMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_nonRealTimeMenuItemActionPerformed
	...
	{// GEN-HEADEREND:event_newMenuItemActionPerformed
		...
		boolean closed = closeScenario(true); // make sure it is okay to close
												// current scenario?
		...
		realTimeAnimationRefreshRateMs = 1000; // refresh rate for real time
												// animation
	...
	{// GEN-HEADEREND:event_simPropMenuItemActionPerformed
		...
		JInternalFrame iframe = new JInternalFrame("Simulation Properties",
				true, true, true, true);
		...
		iframe.setFrameIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/logo/JSatTrakLogo_16.png")));
	...
	private void twoDwindowPropMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_twoDwindowPropMenuItemActionPerformed
	...
	private void openSatBrowserButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_openSatBrowserButtonActionPerformed
	...
	private void recenterToggleButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_recenterToggleButtonActionPerformed
	...
	private void zoomOuttoggleButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_zoomOuttoggleButtonActionPerformed
	...
	private void zoomIntoggleButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_zoomIntoggleButtonActionPerformed
	{// GEN-HEADEREND:event_zoomIntoggleButtonActionPerformed
		// make other zoom button unselected
		zoomOuttoggleButton.setSelected(false); // does this trigger an event on
												// other?, nope
		recenterToggleButton.setSelected(false);
	...
	{// GEN-HEADEREND:event_aboutMenuItemActionPerformed
		// Show about menu
		AboutDialog ad = new AboutDialog(versionString, this,
				gov.nasa.worldwind.Version.getVersion());
	...
	private void localTimeZoneCheckBoxActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_localTimeZoneCheckBoxActionPerformed

		// Set the formatting timezone
		if (localTimeZoneCheckBox.isSelected()) {
			// local time zone
			currentOrekitTime.setDateStringFormat(localTZ);
			// epoch
			scenarioEpochDate.setDateStringFormat(localTZ);
		} else {
			// UTC
			currentOrekitTime.setDateStringFormat(TimeZone.getTimeZone("UTC"));
			// epoch
			scenarioEpochDate.setDateStringFormat(TimeZone.getTimeZone("UTC"));
		}

		// update date box:
		dateTextField.setText(currentOrekitTime.getDateTimeStr());
	...
	{// GEN-HEADEREND:event_dateTextFieldActionPerformed
		// save old time
		double prevJulDate = currentOrekitTime.getJulianDay();
		...
		try {
			currentTimeDate.setTime(dateformatShort1.parse(dateTextField
					.getText()));
			dateTextField.setText(dateformat.format(currentTimeDate.getTime()));
		} catch (Exception e2) {
			try {
				// try reading without the milliseconds
				currentTimeDate.setTime(dateformatShort2.parse(dateTextField
						.getText()));
				dateTextField.setText(dateformat.format(currentTimeDate
						.getTime()));
			} catch (Exception e3) {
		...
		if (dateAccepted) {
			...
			// save
			currentOrekitTime.setCurentOrekitTime(currentTimeDate);
//			currentJulianDate.set(currentTimeDate.getTimeInMillis());
			...
			// check that time change wasn't too great for resetting ground
			// track
			double timeDiffDays = Math.abs(currentOrekitTime.getJulianDay()
					- prevJulDate); // in days
	...
	private void decementTimeStepButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_decementTimeStepButtonActionPerformed
	...
	private void incrementTimeStepButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_incrementTimeStepButtonActionPerformed
	...
	{// GEN-HEADEREND:event_resetTimeButtonActionPerformed
		...
		if (epochTimeEqualsCurrentTime) {
			...
			currentOrekitTime.update2CurrentTime(); // update();// =
													// getCurrentJulianDate();
													// // ini time
		} else {
			// update to epoch
			currentOrekitTime.setCurentOrekitTime(scenarioEpochDate.getCurrentOrekitTime());
		}
		...
		for (AbstractSatellite sat : satHash.values()) {
			...
			sat.getSatOptions().setSatColor(
					sat.getSatOptions().getTrueSatColor());
			sat.getSatOptions().setGroundTrackColor(
					sat.getSatOptions().getTrueSatColor());
	...
	private void runAnimation(final boolean eventDetector) {
		...
		playTimer = new Timer(animationRefreshRateMs, new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				// SEG - if update took a long time reduce the timers repeat
				// interval
	...
	// input change in time in days, checks to see if ground tracks need to be
	// updated
	...
	private void realTimeModeCheckBoxActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_realTimeModeCheckBoxActionPerformed
	{// GEN-HEADEREND:event_realTimeModeCheckBoxActionPerformed
		...
		if (realTimeModeCheckBox.isSelected()) {
			...
			animationRefreshRateMs = realTimeAnimationRefreshRateMs; // set real
																		// time
																		// refresh
																		// rate
		} else {
			...
			animationRefreshRateMs = nonRealTimeAnimationRefreshRateMs; // non
																		// real
																		// time
																		// refresh
																		// rate
	...
	private void stepForwardTimeButtonActionPerformed(
	...
	private void new2DWindowMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_new2DWindowMenuItemActionPerformed
	...
	private JInternalFrame createNew2dWindow() {
		...
		J2DEarthPanel newPanel = new J2DEarthPanel(satHash, gsHash,
				zoomIntoggleButton, zoomOuttoggleButton, recenterToggleButton,
				currentOrekitTime, sun, this);
	...
	private JInternalFrame createNew3DInternalWindow() {
		...
		J3DEarthInternalPanel newPanel = new J3DEarthInternalPanel(iframe,
				satHash, gsHash, currentOrekitTime.getModifiedJulianDay(), this);
	...
	public JDialog createNew3dWindow() {
		...
		// JInternalFrame(windowName,true,true,true,true);
		// JFrame iframe = new JFrame(windowName);
		JDialog iframe = new JDialog(this, windowName, false); // parent, title,
																// modal

		// create 3D Earth Panel:
		J3DEarthPanel newPanel = new J3DEarthPanel(iframe, satHash, gsHash,
				currentOrekitTime.getModifiedJulianDay(), this);
		...
		// set icon
		iframe.setIconImage(Toolkit.getDefaultToolkit().getImage(
				getClass().getResource("/icons/logo/JSatTrakLogo_16.png")));

		// //// SEG 23 March 2009 ///////////////////////////////
		// Check look and feel to see if this JDialog should have a deocrated
		// window
	...
	// update time and take an animation step
	public void updateTime(boolean eventDetector) {
		...
		if (realTimeModeCheckBox.isSelected()) {
			...
			currentOrekitTime.addSeconds(currentPlayDirection
					* animationSimStepSeconds);
		}

		// update sun position
		sun.setCurrentMJD(currentOrekitTime.getModifiedJulianDay());
		...
		// update date box:
		dateTextField.setText(currentOrekitTime.getDateTimeStr());// String.format("%tc",cal)
																	// );

		try {
			// now propogate all satellites to the current time
			for (AbstractSatellite sat : satHash.values()) {
				sat.propogate2JulDate(currentOrekitTime.getCurrentOrekitTime(),
						eventDetector);
			} // propgate each sat
		...
		for (GroundStation gs : gsHash.values()) {
			gs.setCurrentJulianDate(currentOrekitTime.getJulianDay());

			// test look angles
			// SatelliteProps sp= satHash.get("ISS (ZARYA)             ");
			// double[] aer = gs.calculate_AER(sp.getJ2000Position());
			//
			// System.out.println("AER: " + aer[0] + ", " + aer[1] + ", " +
			// aer[2]);
		...
		for (J3DEarthPanel threeDPanel : threeDWindowVec) {
			threeDPanel.setMJD(currentOrekitTime.getModifiedJulianDay());
		}
		...
		for (JSatTrakTimeDependent tdo : timeDependentObjects) {
			if (tdo != null) {
				tdo.updateTime(currentOrekitTime, satHash, gsHash);
			}
		}
		...
		for (JTrackingPanel tp : trackingWindowVec) {
			tp.updateTime(currentOrekitTime.getDateTimeStr());
		}

	} // update time

	public TimeOrekit getCurrentJulianDay() {
	...
	public AbsoluteDate getCurrentJulTime() {
		return currentOrekitTime.getCurrentOrekitTime();
	}
	...
	public void showSatBrowserInternalFrame() {
		...
		// JSatBrowser satBrowser = new JSatBrowser(this, false, this); //
		// non-modal version
		//
		// // create new internal frame window
		...
		// {
		// iframe.setJMenuBar(satBrowser.getJMenuBar()); // get menu bar too!
		// (if there is one)
		// }
	...
	private void newInternal3DWindowMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_newInternal3DWindowMenuItemActionPerformed
	{// GEN-HEADEREND:event_newInternal3DWindowMenuItemActionPerformed

		// Easter Egg Sorta -- holding the control key while opening a 3D window
		// opens the window in a dialog - though window is not saved
		// the dialog version uses a faster WWJ rendering class!!!
		if (evt.getModifiers() == 18) // 18 = control key held down (for win at
										// least!) this could be problematic
		{
			createNew3dWindow();
			return;
		}
	...
	private void showGSBrowserMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_showGSBrowserMenuItemActionPerformed
	...
	private void newExternal3DWindowMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_newExternal3DWindowMenuItemActionPerformed
	...
	private void commandShellMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_commandShellMenuItemActionPerformed
	{// GEN-HEADEREND:event_commandShellMenuItemActionPerformed

		JDialog iframe = new JDialog(this, "Command Shell", false); // parent,
																	// title,
																	// modal
	...
	private void commandShellDesktopMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_commandShellDesktopMenuItemActionPerformed
	{// GEN-HEADEREND:event_commandShellDesktopMenuItemActionPerformed
		// open the command desktop - all kinds of freatures for editing/testing
		// scripts
		// and a class browser
		try {
	...
	private void commandServerMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_commandServerMenuItemActionPerformed
	...
	private void commandClientMenuItemActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_commandClientMenuItemActionPerformed
	...
	private void createMovieButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_createMovieButtonActionPerformed
		...
		// figure out what window is selected (if any), get location on screen /
		// width / height

		// if no frames are selected - use entire application as component
		if (mainDesktopPane.getSelectedFrame() == null) {
			panel = new JCreateMovieDialog(this, false, this.getContentPane(),
					this, "JSatTrak App");
		} else if (mainDesktopPane.getSelectedFrame().getContentPane() instanceof J3DEarthComponent) { // 3d
																										// window

			panel = new JCreateMovieDialog(this, false,
					(J3DEarthComponent) mainDesktopPane.getSelectedFrame()
							.getContentPane(), this);
		} else if (mainDesktopPane.getSelectedFrame().getContentPane() instanceof J2DEarthPanel) { // if
																									// a
																									// 2d
																									// window

			panel = new JCreateMovieDialog(this, false,
					(J2DEarthPanel) mainDesktopPane.getSelectedFrame()
							.getContentPane(), this);
		} else // default container
		{
			panel = new JCreateMovieDialog(this, false, mainDesktopPane
					.getSelectedFrame().getContentPane(), this, mainDesktopPane
					.getSelectedFrame().getName());
		}
	...
	private void movieWholeAppMenuItemActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_movieWholeAppMenuItemActionPerformed
		JCreateMovieDialog panel = new JCreateMovieDialog(this, false,
				this.getContentPane(), this, "JSatTrak App");
	...
	private void coverageMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_coverageMenuItemActionPerformed
		...
		if (coverageAnalyzer == null) {
			coverageAnalyzer = new CoverageAnalyzer(currentOrekitTime); // setup
																		// new
																		// analyzer
		...
		// show satellite browser window
		// JTrackingToolSelector trackingBrowser = new
		// JTrackingToolSelector(satHash, gsHash, this); // non-modal version
		JCoverageDialog coverageBrowser = new JCoverageDialog(coverageAnalyzer,
				currentOrekitTime, this, satHash, twoDWindowVec);
	...
	private void toolbar3DWindowButtonActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_toolbar3DWindowButtonActionPerformed
		createNew3dWindow();
	...
	public JTrackingPanel openTrackingToolSelectorWindow() {
		...
		// JTrackingToolSelector trackingBrowser = new
		// JTrackingToolSelector(satHash, gsHash, this); // non-modal version
		JTrackingPanel trackingBrowser = new JTrackingPanel(satHash, gsHash,
				currentOrekitTime.getDateTimeStr(), currentOrekitTime, this);
	...
	public void showGSBrowserInternalFrame() {
		...
		// show satellite browser window
		JGroundStationBrowser gsBrowser = new JGroundStationBrowser(this); // non-modal
																			// version
	...
	public void internalFrameClosed(InternalFrameEvent e) {
		try // try to cast frame contents as a J2DEarthPanel -- to see if it is
			// that type
		{
			J2DEarthPanel closedEarthPanel = (J2DEarthPanel) (((JInternalFrame) e
					.getSource()).getContentPane());
			twoDWindowVec.remove(closedEarthPanel);
		} catch (Exception ee) {
			try {
				// if not see if it was a SatPropertyPanel
				SatPropertyPanel closedPropPanel = (SatPropertyPanel) (((JInternalFrame) e
						.getSource()).getContentPane());
				...
				try {
					// if not see if it was a SatPropertyPanel
					SatSettingsPanel closedPropPanel = (SatSettingsPanel) (((JInternalFrame) e
							.getSource()).getContentPane());
					...
					try {
						...
						J3DEarthInternalPanel closedPropPanel = (J3DEarthInternalPanel) (((JInternalFrame) e
								.getSource()).getContentPane());
						...
						try {
							// if not see if it was a J3DEarthInternalPanel
							JTrackingPanel closedPropPanel = (JTrackingPanel) (((JInternalFrame) e
									.getSource()).getContentPane());
	...
	public void addInternalFrame(JInternalFrame iframe, int xloc, int yloc) {
		...
		// Figure out which kind of internal frame was added, and add it to
		// Vector
		try {
			// is it a satellite Propery window?
			SatPropertyPanel closedPropPanel = (SatPropertyPanel) (iframe
					.getContentPane());
	...
	public void addSat2ListByName(String satName) {
		...
		if (newTLE != null) {
			...
			try {
				CustomSatellite prop = new CustomSatellite(newTLE.getSatName(),
						newTLE.getLine1(), newTLE.getLine2(), new SatOption());

				// add sat to list
				objListPanel.addSat2List(prop);
			} catch (Exception e) {
				// do nothing just ignore the bad satellite trying to be added
				// (and don't add it)
			}
		}
	...
	public void createScreenCapture() {
		try {
			...
			// figure out what window is selected (if any), get location on
			// screen / width / height
			if (mainDesktopPane.getSelectedFrame() == null) {
				JOptionPane.showInternalMessageDialog(mainDesktopPane,
						"A window must be selected to perform a screenshot",
						"ERROR", JOptionPane.ERROR_MESSAGE);
				return;
			}
			pt = mainDesktopPane.getSelectedFrame().getContentPane()
					.getLocationOnScreen();
			width = mainDesktopPane.getSelectedFrame().getContentPane()
					.getWidth();
			height = mainDesktopPane.getSelectedFrame().getContentPane()
					.getHeight();

			// if a 2d window was selected do a little more math to just get to
			// map
			...
			if (height <= 0 || width <= 0) {
				// no screen shot
				JOptionPane.showInternalMessageDialog(mainDesktopPane,
						"A Screenshot was not possible - too small of size",
						"ERROR", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			// BufferedImage screencapture = new Robot().createScreenCapture(
			// new Rectangle( mainFrame.getX()+viewsTabbedPane.getX(),
			// mainFrame.getY(),
			// viewsTabbedPane.getWidth(), mainFrame.getHeight() ) );
			// scree shot of just window
			BufferedImage screencapture = new Robot()
					.createScreenCapture(new Rectangle(pt.x, pt.y, width,
							height));

			// Create a file chooser
			final JFileChooser fc = new JFileChooser();
			jsattrak.utilities.CustomFileFilter pngFilter = new jsattrak.utilities.CustomFileFilter(
					"png", "*.png");
			fc.addChoosableFileFilter(pngFilter);
			jsattrak.utilities.CustomFileFilter jpgFilter = new jsattrak.utilities.CustomFileFilter(
					"jpg", "*.jpg");
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				// addMessagetoLog("Screenshot saved: " +
				...
				Exception e = SaveImageFile.saveImage(fileExtension, file,
						screencapture, 0.9f); // the last one is the compression
												// quality (1=best)
				if (e != null) {
					System.out.println("ERROR SCREEN CAPTURE:" + e.toString());
					JOptionPane.showInternalMessageDialog(mainDesktopPane,
							"ERROR SCREEN CAPTURE:" + e.toString(), "ERROR",
							JOptionPane.ERROR_MESSAGE);
					return;
				}
			...
			JOptionPane.showInternalMessageDialog(mainDesktopPane,
					"ERROR SCREEN CAPTURE:" + e4.toString(), "ERROR",
					JOptionPane.ERROR_MESSAGE);
		}
	} // createScreenCapture
	...
	public void open2dWindowOptions() {
		if (mainDesktopPane.getSelectedFrame() == null
				|| !(mainDesktopPane.getSelectedFrame().getContentPane() instanceof J2DEarthPanel)) {
			JOptionPane.showInternalMessageDialog(mainDesktopPane,
					"A 2D window must be selected.", "ERROR",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		J2DEarthPanel panel = (J2DEarthPanel) mainDesktopPane
				.getSelectedFrame().getContentPane();

		Point pt = mainDesktopPane.getSelectedFrame().getLocation();

		TwoDViewPropertiesPanel propPanel = new TwoDViewPropertiesPanel(panel,
				this);

		// create new internal frame window
		JInternalFrame iframe = new JInternalFrame("2D Earth Window Prop.",
				true, true, true, true);
	...
	public void saveApp(String fileName) {
		...
		try {
			...
			// SEG - added in version 4.0--- using zip format to save files,
			...
			JOptionPane.showMessageDialog(this,
					"Error Saving File: " + e.toString(), "SAVE ERROR",
					JOptionPane.ERROR_MESSAGE);
	...
	public boolean closeScenario(boolean askIfSure) {
		...
		// JOptionPane.showMessageDialog(app, message, "ERROR",
		// JOptionPane.ERROR_MESSAGE);
		if (askIfSure) {
			int answer = JOptionPane.showConfirmDialog(this,
					"Are you sure you want to close the current scenario?",
		...
		for (JInternalFrame jif : mainDesktopPane.getAllFrames()) {
			if (!(jif.getContentPane() instanceof JObjectListPanel)) {
				...
				((JObjectListPanel) jif.getContentPane()).refreshObjectList(); // refresh
																				// the
																				// list
																				// (clear
																				// it)
	...
	public void openFile() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			boolean closed = closeScenario(true); // make sure it is okay to
			...
			try {
				...
				// ObjectInputStream obj_in = new JSX.ObjectReader(f_in);// new
				// ObjectInputStream(f_in); // JSX
				//
				// // Read an object
				...
				// InputStreamReader(new FileInputStream(getFileSaveAs()),
				// "UTF8"));
				//
				// XStream xstream = new XStream();
				...
				if (obj instanceof JstSaveClass) // it better be
				{
					...
					satHash.clear();
					Hashtable<String, CustomSatellite> tempHash = openClass
							.getSatHash();
					for (String key : tempHash.keySet()) {
						satHash.put(key, tempHash.get(key)); // copy manually
						satHash.get(key)
								.setUse3dModel(
										satHash.get(key).getSatOptions()
												.isUse3dModel()); // auto-loads
																	// 3D models
																	// if they
																	// are used
					}
					...
					gsHash.clear();
					Hashtable<String, GroundStation> tempHash2 = openClass
							.getGsHash();
					...
					// load other data
					this.currentOrekitTime = openClass.getCurrentJulianDate();
					this.realTimeAnimationRefreshRateMs = openClass
							.getRealTimeAnimationRefreshRateMs();
					this.nonRealTimeAnimationRefreshRateMs = openClass
							.getNonRealTimeAnimationRefreshRateMs();
					this.currentTimeStepSpeedIndex = openClass
							.getCurrentTimeStepSpeedIndex();
					this.setLocalTimeModeSelected(openClass
							.isLocalTimeZoneSelected());
					this.setRealTimeMode(openClass.isRealTimeMode());

					this.epochTimeEqualsCurrentTime = openClass
							.isEpochTimeEqualsCurrentTime();
					if (!epochTimeEqualsCurrentTime) {
						this.scenarioEpochDate = openClass
								.getScenarioEpochDate();
					}

					// update parameters based on these new parameters
					updateTimeStepsDataGUI();
					setRealTimeAnimationRefreshRateMs(realTimeAnimationRefreshRateMs); // will
																						// reset
																						// all
																						// internal
																						// values
					...
					satListInternalFrame.setBounds(openClass.getSatListX(),
							openClass.getSatListY(),
							openClass.getSatListWidth(),
							openClass.getSatListHeight());
					...
					// set ofline mode
					try {
						gov.nasa.worldwind.WorldWind.getNetworkStatus()
								.setOfflineMode(wwjOfflineMode);
					} catch (Exception ee) {
					}
					...
					if (coverageAnalyzer != null) {
						timeDependentObjects.add(coverageAnalyzer); // add
																	// object to
																	// time
																	// updates
					}
					...
					for (J2DEarthPanelSave j2dp : openClass
							.getTwoDWindowSaveVec()) {
						JInternalFrame iframe = createNew2dWindow(); // create
																		// new
																		// window
						J2DEarthPanel newPanel = twoDWindowVec.lastElement(); // get
																				// window
																				// just
																				// created

						j2dp.copySettings2PanelAndFrame(newPanel, iframe); // copy
																			// settings
																			// to
																			// this
																			// window
					...
					for (SatPropertyPanelSave propPan : openClass
							.getSatPropWindowSaveVec()) {
						...
						String windowName = prop.getName().trim(); // set name -
																	// trim
																	// excess
																	// spaces

						// create new internal frame window
						JInternalFrame iframe = new JInternalFrame(windowName,
								true, true, true, true);

						iframe.setContentPane(newPanel); // set contents pane
						iframe.setSize(propPan.getWidth(), propPan.getHeight()); // set
																					// size

						iframe.setVisible(true);
						addInternalFrame(iframe, propPan.getXPos(),
								propPan.getYPos());
					}

					// PUT BEFORE 3D windows so they load correctly though data
					// in them might be funny?
					// update GUI -- this is messing up the 3D views.. they need
					// some time to render
					forceRepainting(true); // force repaint and regeneration of
											// data

					// create all needed 3D windows:

					// create all 3D external windows needed
					for (J3DEarthlPanelSave j3dp : openClass
							.getThreeDExtWindowSaveVec()) {
						Container iframe = createNew3dWindow();
						J3DEarthPanel newPanel = threeDWindowVec.lastElement(); // get
																				// window
																				// just
																				// created

						j3dp.copySettings2PanelAndFrame(newPanel, iframe); // copy
																			// settings
																			// to
																			// this
																			// window
					...
					for (J3DEarthlPanelSave j3dp : openClass
							.getThreeDWindowSaveVec()) {
						JInternalFrame iframe = createNew3DInternalWindow(); // create
																				// new
																				// window
						J3DEarthInternalPanel newPanel = threeDInternalWindowVec
								.lastElement(); // get window just created
					...
					try {
						...
						this.setSize(openClass.getAppWidth(),
								openClass.getAppHeight());
						this.repaint();
					} catch (Exception e) {
						System.out
								.println("Saved File didn't have any information on app size or location.");
					}
					...
					try {
						...
						if (laf != null) {
							...
							System.out
									.println("Saved File didn't have any look and feel data.");
						}
					} catch (Exception e) {
						System.out
								.println("Saved File didn't have any or has invalid look and feel data.");
					}
					...
					JOptionPane
							.showMessageDialog(
									this,
									"Error Opening File: Incorrect file format, may be corrupt or an old version",
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(this,
						"Error Opening File: " + e.toString(), "OPEN ERROR",
						JOptionPane.ERROR_MESSAGE);
	...
	public int[] getSatListWHXY() {
		return new int[] { satListInternalFrame.getWidth(),
				satListInternalFrame.getHeight(), satListInternalFrame.getX(),
				satListInternalFrame.getY() };
	}
	...
	public void windowClosing(WindowEvent e) {
		...
		try // try to cast frame contents as a J3DEarthPanel -- to see if it is
			// that type
		{
			J3DEarthPanel closedEarthPanel = (J3DEarthPanel) (((JDialog) e
					.getSource()).getContentPane());
			threeDWindowVec.remove(closedEarthPanel); // remove from Vector

			// System.out.println("3D Window Count = " +
			// threeDWindowVec.size());

			System.gc(); // force garbage collection after getting rid of that
							// window
	...
	public void setTime(TimeOrekit time) {
		currentOrekitTime = time;
	...
	public void setTime(double julianDate) {
		...
		this.currentOrekitTime.setCurentOrekitTime(gc);
	}
	...
	/**
	 * Sets the text of the status bar and resets the timer for the message to
	 * be erased
	 * 
	 * @param msg
	...
	public// WWJ online/offline mode
	 ...
	 *            full or relative path to the script file to be executed
	 */
	public void runScript(String scriptFilePath) {
		...
		try {
			...
			System.out.println("Error running plugin script (" + scriptFilePath
					+ "): " + ee.toString());
			JOptionPane.showMessageDialog(this, "Error running plugin script ("
					+ scriptFilePath + "): \n" + ee.toString(),
					"Pluging Error", JOptionPane.ERROR_MESSAGE);
		}
	} // runScript
	...
	public void addCustomSat() {
		try {
			...
			objListPanel.openCurrentOptions(new MissionTableModel(
					scenarioEpochDate));
		} catch (OrekitException e) {
			JOptionPane.showMessageDialog(this, e.getMessage(), "Error",
					JOptionPane.ERROR_MESSAGE);
	...
	public J2DEarthPanel getNonDisplayed2DEarthPanel(int height) {
		int width = 2 * height;
		// create panel
		J2DEarthPanel earthPanel = new J2DEarthPanel(satHash, gsHash,
				zoomIntoggleButton, zoomOuttoggleButton, recenterToggleButton,
				currentOrekitTime, sun, this);
		// insure that the panel is the correct size and all background painting
		// has been done
	 ...
	 * 
	 *            if run with no command line parameters the GUI is displayed if
	 *            run with a command line argument with the path of a script it
	 *            will be executed example : java JSatTrak runme.bsh >
	 *            satLog.txt (will run the script runme.bsh and save output to
	 *            the satLog.txt file)
	 */
	public static void main(final String args[]) {
		...
		if (orekitData.exists() || !orekitData.isDirectory()) {
			...
			System.err.println(orekitData.getAbsolutePath()
					+ " not found, aborting");
			System.exit(1);
		}
		...
		{
			// see if command line is a exisiting file (assume it is a .bsh file
			// and try to run it)
			...
			if (inScriptFile.exists()) {
				...
				try {
					...
					System.err
							.println("Error running script: " + ee.toString());
				}
