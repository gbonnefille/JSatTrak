...
public class J2dEarthLabel2 extends JLabel implements java.io.Serializable {
	...
	private double aspectRatio = 2.0;// aspect ratio of the image in the frame
										// (should equal value in J2EarthPanel)
	...
	private double zoomIncrementMultiplier = 2.0; // how much to zoom at a time,
													// e.g., 2= zoom in a factor
													// of 2
	...
	private int numPtsSunFootPrint = 61; // used to be 101, (51 is too low -
											// errors on which side is filled
											// in)
	...
	// hidden option -- mostly for debug -- Toggle this using "f" key when a 2D
	// window is active
	...
	public J2dEarthLabel2(ImageIcon image, double aspect,
		...
		// // TEMP try out the LandMassRegions -- move this to top class, so
		// each map doesn't have their own? or leave it this way so each can be
		// customized
	...
	public void paintComponent(Graphics g) {
		// paint the "Earth Lights" where the earth is in shadow - if effect is
		// choosen
		...
		finishPainting(g); // for profiling (otherwise move method back to this
							// point

	} // paintComponent

	// method to do all the painting so profiler accounts for it correctly
	// hmm... use The conclusions are that drawPolyline() is about 3-6 times
	// faster than drawLine()
	// http://www.particle.kth.se/~fmi/kurs/PhysicsSimulation/Lectures/12A/timing.html
	// also replace use of polygon... to drawPolygon(int[] xPoints, int[]
	// yPoints, int nPoints) and fillpolygon(int[]...) ... so they share the
	// same data
	private void finishPainting(Graphics g) {
		...
		for (JSatTrakRenderable renderable : renderableObjects) {
			renderable.draw2d(g2, this, w, h, imageWidth, imageHeight,
					zoomFactor, centerLat, centerLong);
		}
		...
		if (drawSun) {
			...
			// double[] lla = GeoFunctions.GeodeticJulDate(
			// sun.getOpositeSunPositionMOD() ,currentTime.getJulianDate());
			double[] lla = sun.getCurrentDarkLLA();
			drawFootPrint(g2, lla[0], lla[1], lla[2], true, sunColor, sunColor,
					sunAlpha, numPtsSunFootPrint); // draw footprint
		...
		if (showLatLong) {
			for (int lat = -90; lat <= 90; lat += 30) {
				...
				// g2.drawLine(xy_old[0],xy_old[1],xy[0],xy[1]); // draw a line
				// across the map
				g2.drawPolyline(new int[] { xy_old[0], xy[0] }, new int[] {
						xy_old[1], xy[1] }, 2); // faster
			}
		...
		for (GroundStation gs : gsHash.values()) // search through all sat nodes
		{
			...
			if (gs.isShow2D()) {
				...
				int[] gs_xy = findXYfromLL(gs.getLatitude(), gs.getLongitude(),
				...
				if (gs.isShow2DName()) {
					// show name?
					g2.drawString(gs.getStationName().trim(), gs_xy[0] + sz / 2
							+ 3, gs_xy[1] + 4);
		...
		for (AbstractSatellite sat : satHash.values()) // search through all sat
														// nodes
		{
			if (sat.getSatOptions().isShowGroundTrack()
					&& sat.getSatOptions().isGroundTrackIni()
					&& sat.getSatOptions().isPlot2d()) {
				// System.out.println("Plotting node:"+i);
				// okay plot LLA's of the satellite
				g2.setPaint(sat.getSatOptions().getGroundTrackColor());
				...
				if (sat.getNumGroundTrackLeadPts() > 1) {
					...
					xy_old = findXYfromLL(LLA[0] * 180.0 / Math.PI, LLA[1]
							* 180.0 / Math.PI, w, h, imageWidth, imageHeight);
					LLA_old[0] = LLA[0];
					LLA_old[1] = LLA[1];
					// LLA_old[2] = LLA[2];

					// faster performance to draw allpoints at once useing
					// drawPolyLine
					int[] xPts = new int[sat.getNumGroundTrackLeadPts()];
					int[] yPts = new int[sat.getNumGroundTrackLeadPts()];
					int ptsCount = 0; // points to draw stored up (reset when
										// discontinutiy is hit)
					...
					for (int j = 1; j < sat.getNumGroundTrackLeadPts(); j++) {
						...
						xy = findXYfromLL(LLA[0] * 180.0 / Math.PI, LLA[1]
								* 180.0 / Math.PI, w, h, imageWidth,
								imageHeight);

						// check to see if Longitude flipped sign But not near
						// origin
						// if( (LLA[1] > 0 && LLA_old[1] > 0) || (LLA[1] < 0 &&
						// LLA_old[1] < 0) || Math.abs(LLA[1]) < 1.0 )
						if (!(nanDbl.equals(LLA_old[0]) || nanDbl
								.equals(LLA[0]))) // make sure they are not NAN
													// (not in time)
						{

							// line segment is normal (doesn't span map
							// disconnect)
							if (Math.abs(LLA[1] - LLA_old[1]) < 4.0) {
								// old slow way:
								// g2.drawLine(xy_old[0], xy_old[1], xy[0],
								// xy[1]); // draw a line across the map
								...
								// draw this line segment next time, jump from
								// side to side
								double newLat = linearInterpDiscontLat(
										LLA_old[0], LLA_old[1], LLA[0], LLA[1]);

								// get xy points for both the old and new side
								// (positive and negative long)
								int[] xyMid_pos = findXYfromLL(newLat * 180.0
										/ Math.PI, 180.0, w, h, imageWidth,
										imageHeight);
								int[] xyMid_neg = findXYfromLL(newLat * 180.0
										/ Math.PI, -180.0, w, h, imageWidth,
										imageHeight);

								// draw 2 lines - one for each side of the date
								// line
								if (LLA_old[1] > 0) // then the old one is on
													// the positive side
								{
									// old slow way
									// g2.drawLine(xy_old[0], xy_old[1],
									// xyMid_pos[0], xyMid_pos[1]);
									// g2.drawLine(xy[0], xy[1], xyMid_neg[0],
									// xyMid_neg[1]);
								...
								{
									// old slow way
									// g2.drawLine(xy[0], xy[1], xyMid_pos[0],
									// xyMid_pos[1]);
									// g2.drawLine(xy_old[0], xy_old[1],
									// xyMid_neg[0], xyMid_neg[1]);
					...
					for (int j = 1; j < sat.getNumGroundTrackLagPts(); j++) {
													...
													// (not in time)
						{
							if (Math.abs(LLA[1] - LLA_old[1]) < 4.0) {
								...
								// System.out.println("xy: (" + xy_old[0] +","
								// +xy_old[1] +"),(" +xy[0] +"," +xy[1]+"), j="
								// + j + "LLA_old,new:" +LLA_old[1] + ", "
								// +LLA[1] ); // draw a line across the map
							} else {
								// draw this line segment next time, jump from
								// side to side
								double newLat = linearInterpDiscontLat(
		...
		for (AbstractSatellite sat : satHash.values()) // search through all sat
														// nodes
		{
			if (sat.getSatOptions().isPlot2d()) // if option to plot is on
			{
				// draw a small circle around where the current position of the
				// sat is

				g2.setPaint(sat.getSatOptions().getSatColor());

				double lat = sat.getLatitude();
				double lon = sat.getLongitude();

				// System.out.println("Lat/Lon =" + lat*180.0/Math.PI + "/" +
				// lon*180.0/Math.PI);

				// find the drawing location
				xy = findXYfromLL(lat * 180.0 / Math.PI, lon * 180.0 / Math.PI,
						w, h, imageWidth, imageHeight);

				// drawOval
				g2.fillOval(xy[0] - 5, xy[1] - 5, 10, 10); // not sure what the
															// arguments are?

				// add name if required
				if (sat.getSatOptions().isShowName2D()) {
				...
				if (sat.isEventDetected() && !sat.getEventPositions().isEmpty()) {
					...
					Iterator<double[]> eventPositionIterator = sat
							.getEventPositions().iterator();
					Iterator<String> eventNameIterator = sat.getEventName()
							.iterator();
					g2.setPaint(sat.getSatOptions().getGroundTrackColor());
					...
					while (eventPositionIterator.hasNext()) {
						...
						int[] event_xy = findXYfromLL(eventPosition[1] * 180.0
								/ Math.PI, eventPosition[2] * 180.0 / Math.PI,
								w, h, imageWidth, imageHeight);

						int sz = sat.getEventPosition2DPixelSize();

						g2.fillOval(event_xy[0] - sz / 2, event_xy[1] - sz / 2,
								sz, sz);

						// g2.fillRect(event_xy[0] - sz / 2, event_xy[1] - sz /
						// 2, sz, sz);

						// if(gs.isShow2DName())
						// {
						// // show name?
						g2.drawString(eventName, event_xy[0] + sz / 2 + 3,
								event_xy[1] + 4);
						//
						// } // if show name
		...
		for (AbstractSatellite sat : satHash.values()) // search through all sat
														// nodes
		{
			if (sat.getSatOptions().isPlot2d()
					&& sat.getSatOptions().isPlot2DFootPrint()) {
				...
				// draw the foot print
				drawFootPrint(g2, lat, lon, alt, sat.getSatOptions()
						.isFillFootPrint(), sat.getSatOptions().getSatColor(),
						sat.getSatOptions().getSatColor(), 0.2f, sat
								.getSatOptions().getNumPtsFootPrint()); // draw
																		// footprint
				// Graphics2D g2, double lat, double lon, double alt, boolean
				// fillFootPrint, Color outlineColor, Color FillColor,
				// alpha(0.2f) int numPtsFootPrint
		...
		if (zoomFactor > 1.0001) {
			...
			if (getWidth() / getHeight() > aspectRatio) {
				// draw in ends
				g2.fillRect(0, 0, (int) ((getWidth() - imageWidth) / 2.0),
						getHeight());
				g2.fillRect(
						(int) ((getWidth() - imageWidth) / 2.0 + imageWidth),
						0, (int) ((getWidth() - imageWidth) / 2.0), getHeight());
			} else {
				// draw on top/bottom
				g2.fillRect(0, 0, getWidth(),
						(int) ((getHeight() - imageHeight) / 2.0));
				g2.fillRect(
						0,
						(int) ((getHeight() - imageHeight) / 2.0 + imageHeight),
						getWidth(), (int) ((getHeight() - imageHeight) / 2.0));
			}
		...
		if (showDateTime) {
			...
			g2.drawString(
					currentTime.getDateTimeStr(),
					(int) ((getWidth() - imageWidth) / 2.0) + xDateTimeOffset,
					(int) ((getHeight() - imageHeight) / 2.0 + imageHeight - yDateTimeOffset));
		...
		if (showFPS) {
			g2.setPaint(dateTimeColor);
			g2.drawString(
					"FPS: " + df.format(earthPanel.getApp().getFpsAnimation()),
					(int) (getWidth() - 75), (int) ((getHeight() - imageHeight)
							/ 2.0 + imageHeight - yDateTimeOffset));
		}
	} // finish painting
	...
	private double linearInterpDiscontLat(double lat1, double long1,
			double lat2, double long2) {
		// one longitude should be negative one positive, make them both
		// positive
		if (long1 > long2) {
	...
	public int[] findXYfromLL(double lat, double lon, int totWidth,
			int totHeight, int imgWidth, int imgHeight) {
		return findXYfromLL(lat, lon, totWidth, totHeight, imgWidth, imgHeight,
				zoomFactor, centerLat, centerLong);
	}

	// includs calculations for zoom Facter, central lat and central longitude-
	// input degrees
	public int[] findXYfromLL(double lat, double lon, int totWidth,
			int totHeight, int imgWidth, int imgHeight, double zoomFac,
	...
	// assumes local zoom factor centered centerLat, centerLong
	public double[] findLLfromXY(int x, int y, int totWidth, int totHeight,
			int imgWidth, int imgHeight) {
		return findLLfromXY(x, y, totWidth, totHeight, imgWidth, imgHeight,
				zoomFactor, centerLat, centerLong);
	}

	// find LL from xy position - used for clicking on map: - returns in degrees
	public double[] findLLfromXY(int x, int y, int totWidth, int totHeight,
			int imgWidth, int imgHeight, double zoomFac, double cLat,
			double cLong) {
		...
		ll[1] = (x - (rightX + leftX) / 2.0) / ((rightX - leftX) / longSpan)
				+ cLong; // longitude
	...
	private void drawFootPrint(Graphics2D g2, double lat, double lon,
			double alt, boolean fillFootPrint, Color outlineColor,
			Color FillColor, float alpha, int numPtsFootPrint) {

		// vars: ===============================
		// draw footprints of satellites
		Polygon footprint;
		// varaibles for fixing disconnect around international date line (for
		// filled footprints)
		int disconnectCount = 0; // how many disconnect points, 0=good, 1= far
									// away sat, sees north/south pole, 2= spans
									// date line
		int disconnectIndex1 = 0; // first index of disconnect
		int disconnectIndex2 = 0; // second index of disconnect
		int[] disconnect1pos = new int[2]; // corrected x,y point at each
											// discontinutiy
		...
		// correction to convert latitude from geographic to geo-centric because
		// foot print is created and rotated about center so it uses geocentric
		// rotations
		// for conversion see: http://en.wikipedia.org/wiki/Latitude
		lat = Math.atan(Math.pow(AstroConst.R_Earth_minor
				/ AstroConst.R_Earth_major, 2.0)
				* Math.tan(lat));

		// assumes a spherical Earth - cone half angle from the center of the
		// earth
		// an improvement could find the radius of the earth at the given
		// latitude
		// double lambda0 =
		// Math.acos(AstroConst.R_Earth/(AstroConst.R_Earth+alt));
		// improvement - 31 March 2009 - SEG (though still assumes that the
		// visible area is a perfect circle, but that is very close
		// improves accuracy by about 0.02 deg at extremes of a 51.6 deg
		// inclination LEO
		double earthRadiusAtLat = AstroConst.R_Earth_major
				- (AstroConst.R_Earth_major - AstroConst.R_Earth_minor)
				* Math.sin(lat);
		...
		// rotation matrix - 20-March 2009 SEG -- may want to convert LLA from
		// geographic to geocentric -- See correction Above
		// for conversion see: http://en.wikipedia.org/wiki/Latitude
		double[][] M = new double[][] {
				{ Math.cos(beta) * Math.cos(gamma), Math.sin(gamma),
						-Math.sin(beta) * Math.cos(gamma) },
				{ -Math.cos(beta) * Math.sin(gamma), Math.cos(gamma),
						Math.sin(beta) * Math.sin(gamma) },
				{ Math.sin(beta), 0.0, Math.cos(beta) } };
		double theta = 0 + Math.PI / 2.0; // with extra offset of pi/2 so circle
											// starts left of center going
											// counter clockwise
		double phi = lambda0;

		// position - on the surface of the Earth (spherical approx) - depending
		// on how LLA is calculated geodetic or geographic
		// uses the parametric equation of a sphere, around( varies by theta)
		// the bottom of the sphere with the radius of the earth up an angle phi
		// maybe should use mean radius here instead of equatorial?? - doesn't
		// make much difference in the pixel noise
		...
		// original point
		int[] xy_old = findXYfromLL(lla0[0] * 180.0 / Math.PI, lla0[1] * 180.0
				/ Math.PI, w, h, imageWidth, imageHeight);
		xy0[0] = xy_old[0];
		xy0[1] = xy_old[1];
		...
		int ptsCount = 0; // points to draw stored up (reset when discontinutiy
							// is hit)
		// first point
		xPts[ptsCount] = xy_old[0];
		yPts[ptsCount] = xy_old[1];
		...
		for (int j = 1; j < numPtsFootPrint; j++) {
			theta = j * dt + Math.PI / 2.0; // +Math.PI/2.0 // offset so it
											// starts at the side
			// phi = lambda0;
			...
			// System.out.println("ll=" +lla[0]*180.0/Math.PI + "," +
			// (lla[1]*180.0/Math.PI));

			xy = findXYfromLL(lla[0] * 180.0 / Math.PI, lla[1] * 180.0
					/ Math.PI, w, h, imageWidth, imageHeight);
			...
			if (Math.abs(lla[1] - llaOld[1]) < 4.0) {
				...
				double newLat = linearInterpDiscontLat(llaOld[0], llaOld[1],
						lla[0], lla[1]);

				// get xy points for both the old and new side (positive and
				// negative long)
				int[] xyMid_pos = findXYfromLL(newLat * 180.0 / Math.PI, 180.0,
						w, h, imageWidth, imageHeight);
				int[] xyMid_neg = findXYfromLL(newLat * 180.0 / Math.PI,
						-180.0, w, h, imageWidth, imageHeight);
		...
		// draw polygon -- won't work when part of the foot print is on other
		// side
		...
		// still some issues with fill side: see sun /- one minute around - 23
		// Mar 2009 01:14:12.000 MDT
		if (fillFootPrint) {
			Color satCol = FillColor;
			Color transColor = new Color(satCol.getRed() / 255.0f,
					satCol.getGreen() / 255.0f, satCol.getBlue() / 255.0f,
					alpha);
			...
			if (disconnectCount == 0) {
				...
				if (northPoleVisible) {
					...
					ptPos = findXYfromLL(90.0, 180.0, w, h, imageWidth,
							imageHeight);
					ptNeg = findXYfromLL(90.0, -180.0, w, h, imageWidth,
							imageHeight);

					// counter clockwise - add points
					for (int k = 0; k < disconnectIndex1; k++) {
						fullFootPrint.addPoint(footprint.xpoints[k],
								footprint.ypoints[k]);
					}
					fullFootPrint
							.addPoint(disconnect1pos[0], disconnect1pos[1]);
					fullFootPrint.addPoint(ptPos[0], ptPos[1]);
					fullFootPrint.addPoint(ptNeg[0], ptNeg[1]);
					fullFootPrint
							.addPoint(disconnect1neg[0], disconnect1neg[1]);
				...
				// 1 is vertically lower than discon #2) (y is backwards in
				// java)
				if (disconnect1neg[1] >= disconnect2neg[1]) {
					...
					footprintPartLeft.addPoint(disconnect1neg[0],
							disconnect1neg[1]); // first point at left edge
					for (int k = disconnectIndex1; k < disconnectIndex2; k++) {

						footprintPartLeft.addPoint(footprint.xpoints[k],
								footprint.ypoints[k]);
					}
					footprintPartLeft.addPoint(disconnect2neg[0],
							disconnect2neg[1]);
					...
					for (int k = 0; k < disconnectIndex1; k++) {
						...
						footprintPartRight.addPoint(footprint.xpoints[k],
								footprint.ypoints[k]);
					}
					// add disconnect points
					footprintPartRight.addPoint(disconnect1pos[0],
							disconnect1pos[1]);
					footprintPartRight.addPoint(disconnect2pos[0],
							disconnect2pos[1]);
					// fill in last part
				...
				else // non standard discont 1 is above discont 2
				{
					...
					footprintPartLeft.addPoint(disconnect1neg[0],
							disconnect1neg[1]); // left edge top
					footprintPartLeft.addPoint(disconnect2neg[0],
							disconnect2neg[1]); // left edge bottom
					for (int k = disconnectIndex2; k < footprint.npoints; k++) {
	...
	// ======================================================
	// Get footprint polygons - gets the shape(s) of the foot prints - similar
	// to drawFootprint but no drawing
	// Assumes total width and total height match image size, doesn't take into
	// account placement on page...
	// the above assumption is for J2DEarthPanel!! because it on't draws on the
	// image exactly
	public Polygon[] getFootPrintPolygons(double lat, double lon, double alt,
			int numPtsFootPrint) {
		...
		double lambda0 = Math.acos(AstroConst.R_Earth
				/ (AstroConst.R_Earth + alt));
		...
		if (true) {
			// Color satCol = FillColor;
			// Color transColor = new
			// Color(satCol.getRed()/255.0f,satCol.getGreen()/255.0f,satCol.getBlue()/255.0f,alpha);
			// g2.setPaint( transColor );
	...
	// image exactly
	/**
	 * Returns a list of lat/lon (radians) for the footprint TODO: add elevation
	 * limiting constraint!
	 * 
	 ...
	 */
	public static Vector<LatLon> getFootPrintLatLonList(double lat, double lon,
			double alt, int numPtsFootPrint) {
