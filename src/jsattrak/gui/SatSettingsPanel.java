...
public class SatSettingsPanel extends javax.swing.JPanel implements
	...
	JInternalFrame iframe; // used to know what its parent frame is - to close
							// window
	...
	public SatSettingsPanel(AbstractSatellite satProps, JSatTrak app) {
		...
		this.configPanel = new JCustomSatConfigPanel(satProps, app, this);
		int tabIndex = 0;
	 ...
	 */
	public SatSettingsPanel(MissionTableModel MissionTable, JSatTrak app)
			throws OrekitException {
		initComponents();
	...
	private void firstTimeIniSatProps() throws OrekitException {
		...
		lagTrackTextField.setText(satOptions
				.getGroundTrackLagPeriodMultiplier() + "");
	...
	private void iniSatProps() {
		...
		// footprint
		showFootPrintCheckBox.setSelected(satProps.getSatOptions()
				.isPlot2DFootPrint());
		footPrintResTextField.setText(satProps.getSatOptions()
				.getNumPtsFootPrint() + "");
		fillFootprintCheckBox.setSelected(satProps.getSatOptions()
				.isFillFootPrint());

		// ground track
		showGroundTrackCheckBox.setSelected(satProps.getSatOptions()
				.isShowGroundTrack());
		groundTrackResTextField.setText(satProps.getSatOptions()
				.getGrnTrkPointsPerPeriod() + "");
		leadTrackTextField.setText(satProps.getSatOptions()
				.getGroundTrackLeadPeriodMultiplier() + "");
		lagTrackTextField.setText(satProps.getSatOptions()
				.getGroundTrackLagPeriodMultiplier() + "");

		// 3d view
		showOrbit3DCheckBox.setSelected(satProps.getSatOptions()
				.isShow3DOrbitTrace());
		showfootprint3DCheckBox.setSelected(satProps.getSatOptions()
				.isShow3DFootprint());
		showName3DCheckBox.setSelected(satProps.getSatOptions().isShow3DName());
		groundTrack3dCheckBox.setSelected(satProps.getSatOptions()
				.isShowGroundTrack3d());
		eciRadioButton.setSelected(satProps.getSatOptions()
				.isShow3DOrbitTraceECI());
		ecefRadioButton.setSelected(!satProps.getSatOptions()
				.isShow3DOrbitTraceECI());

		// 3d models

		use3DModelCheckBox.setSelected(satProps.getSatOptions().isUse3dModel());
		threeDModelTextField.setText(satProps.getSatOptions()
				.getThreeDModelPath());
		threeDModelTextField.setEditable(satProps.getSatOptions()
				.isUse3dModel()); // set
		// editable
		browseModelButton.setEnabled(use3DModelCheckBox.isSelected());
		modelScaleTextField.setEditable(use3DModelCheckBox.isSelected());
		modelScaleTextField.setText(satProps.getSatOptions()
				.getThreeDModelSizeFactor() + "");
	...
	private void initComponents() {
		...
		jPanel4.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Ground Track"));

		showGroundTrackCheckBox.setText("Show Groundtrack"); // NOI18N
		showGroundTrackCheckBox.setBorder(javax.swing.BorderFactory
		...
		jPanel5.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Footprint"));

		showFootPrintCheckBox.setText("Show Viewable Footprint"); // NOI18N
		showFootPrintCheckBox.setBorder(javax.swing.BorderFactory
				.createEmptyBorder(0, 0, 0, 0));
		...
		fillFootprintCheckBox.setBorder(javax.swing.BorderFactory
				.createEmptyBorder(0, 0, 0, 0));
		...
		jPanel6.setBorder(javax.swing.BorderFactory
				.createTitledBorder("General 2D Options"));

		include2DCheckBox.setText("Include on 2D Map"); // NOI18N
		include2DCheckBox.setBorder(javax.swing.BorderFactory
				.createEmptyBorder(0, 0, 0, 0));
		include2DCheckBox.setMargin(new java.awt.Insets(0, 0, 0, 0));

		colorPanel.setBackground(new java.awt.Color(0, 0, 153));
		colorPanel.setBorder(javax.swing.BorderFactory
				.createLineBorder(new java.awt.Color(0, 0, 0)));

		javax.swing.GroupLayout colorPanelLayout = new javax.swing.GroupLayout(
				colorPanel);
		colorPanel.setLayout(colorPanelLayout);
		colorPanelLayout.setHorizontalGroup(colorPanelLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGap(0, 30, Short.MAX_VALUE));
		colorPanelLayout.setVerticalGroup(colorPanelLayout.createParallelGroup(
		...
		showNameCheckBox.setBorder(javax.swing.BorderFactory.createEmptyBorder(
				0, 0, 0, 0));
	...
	{// GEN-HEADEREND:event_colorButtonActionPerformed
		...
		// show color Dialog
		Color newColor = JColorChooser.showDialog(app,
				"Choose Satellite Color", oldColor);
	...
	{// GEN-HEADEREND:event_applyButtonActionPerformed
		...
		for (InternalFrameListener listener : iframe
	...
	{// GEN-HEADEREND:event_okButtonActionPerformed
		...
		// maybe do this from JSatTrack -- when internal frame is closed of this
		// type and ok was hit?
		okHit = true;
		// force repaint
		app.forceRepainting(updateMaps);
		...
		if (this.dispose) {
			...
			// if the name already exits
			this.dispose = true;
		}

	}// GEN-LAST:event_okButtonActionPerformed

	private void showOrbit3DCheckBoxActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_showOrbit3DCheckBoxActionPerformed
	{// GEN-HEADEREND:event_showOrbit3DCheckBoxActionPerformed
		// TODO add your handling code here:
	}// GEN-LAST:event_showOrbit3DCheckBoxActionPerformed

	private void groundTrack3dCheckBoxActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_groundTrack3dCheckBoxActionPerformed
	...
	private void use3DModelCheckBoxStateChanged(
			javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_use3DModelCheckBoxStateChanged
	...
	private void browseModelButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_browseModelButtonActionPerformed
		...
		String userDir = System.getProperty("user.dir"); // path java was run
															// from
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			try {
				...
				String relPath = RelativePath.getRelativePath(new File(test),
						new File(test2)); // returns conanical rel path

				threeDModelTextField.setText(relPath);

			} catch (Exception e) {
				System.out.println("ERROR Selecting Model File: "
						+ e.toString());
			}
		} // if file choosen
	...
	private boolean saveSettings() {
		...
		String satName = this.configPanel.getSatNameTextField().getText();

		// if nothing given:
		if (satName == null || satName.equalsIgnoreCase("")) {
			// System.out.println("returned");
			app.setStatusMessage("Custom Satellite Canceled: Either by user or not supplying a name.");
			JOptionPane
					.showMessageDialog(
							this,
		...
		// app.setStatusMessage("This custom satellite name already exists.");
		...
		if (newSat) {
			...
			if (!app.objListPanel.satHash.containsKey(satName)) {
				...
				try {
					...
					satProps.propogate2JulDate(app.getCurrentJulTime(), true);

					// add item to the Object list tree
					app.objListPanel.addSat2List(satProps);

					app.setStatusMessage("Custom Satellite Added: " + satName);
				...
				// Sat already exist
				app.setStatusMessage("Satellite already exist, change the name");
				JOptionPane.showMessageDialog(this,
						"Satellite already exist, change the name", "Error",
						JOptionPane.ERROR_MESSAGE);
				this.dispose = false;
				return false;
			}
			...
			// if the user wants to rename the satellite
			if (!satProps.getName().equals(satName)) {
				app.objListPanel.satHash.remove(satProps.getName());
				satProps.setName(satName);
				app.objListPanel.satHash.put(satName, satProps);
				app.objListPanel.refreshObjectList();
			}
		...
		// footprint
		satProps.getSatOptions().setPlot2DFootPrint(
				showFootPrintCheckBox.isSelected());
		try {
			satProps.getSatOptions().setNumPtsFootPrint(
					Integer.parseInt(footPrintResTextField.getText()));
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		satProps.getSatOptions().setFillFootPrint(
				fillFootprintCheckBox.isSelected());
		...
		try {
			...
			satProps.getSatOptions().setGrnTrkPointsPerPeriod(
					Integer.parseInt(groundTrackResTextField.getText()));
		...
		double oldLead = satProps.getSatOptions()
				.getGroundTrackLeadPeriodMultiplier();
		double oldLag = satProps.getSatOptions()
				.getGroundTrackLagPeriodMultiplier();

		try {
			satProps.getSatOptions().setGroundTrackLeadPeriodMultiplier(
					Double.parseDouble(leadTrackTextField.getText()));

			if (oldLead != satProps.getSatOptions()
					.getGroundTrackLeadPeriodMultiplier()) {
		...
		try {
			satProps.getSatOptions().setGroundTrackLagPeriodMultiplier(
					Double.parseDouble(lagTrackTextField.getText()));
			if (oldLag != satProps.getSatOptions()
					.getGroundTrackLagPeriodMultiplier()) {
				// set ground tracks ini to false
				satProps.getSatOptions().setGroundTrackIni2False();
				updateMapData = true;
		...
		// 3d view
		satProps.getSatOptions().setShow3DOrbitTrace(
				showOrbit3DCheckBox.isSelected());
		satProps.getSatOptions().setShow3DFootprint(
				showfootprint3DCheckBox.isSelected());
		satProps.getSatOptions().setShow3DName(showName3DCheckBox.isSelected());
		satProps.getSatOptions().setShowGroundTrack3d(
				groundTrack3dCheckBox.isSelected());
		satProps.getSatOptions().setShow3DOrbitTraceECI(
				eciRadioButton.isSelected());
		...
		if (use3DModelCheckBox.isSelected()) {
			...
			satProps.setThreeDModelSizeFactor(Double
					.parseDouble(modelScaleTextField.getText()));
		}

		return updateMapData;
	} // saveSettings

	public void setIsPropagate(boolean isPropagate) {
