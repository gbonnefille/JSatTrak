...
public class JObjectListInternalFrame extends javax.swing.JInternalFrame {
	...
	public JObjectListInternalFrame(
		...
		topGSTreeNode.setIcon(new ImageIcon(Toolkit.getDefaultToolkit()
				.getImage(
						getClass().getResource(
								"/icons/custom/groundStation.png"))));
		...
		objectTree.setTransferHandler(new ObjectTreeTransferHandler(satHash,
				gsHash, app, topSatTreeNode, topGSTreeNode));
	...
	private void initComponents() {
		...
		setFrameIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/logo/JSatTrakLogo_16.png"))); // NOI18N
		setPreferredSize(new java.awt.Dimension(269, 331));

		removeSatButton.setIcon(new javax.swing.ImageIcon(getClass()
		...
		infoSatButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/other/Information16.gif"))); // NOI18N
		...
		optionsSatButton.setIcon(new javax.swing.ImageIcon(getClass()
		...
		addCustomSatButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/custom/sat_icon_cst.png"))); // NOI18N
		addCustomSatButton
		...
		addSatButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/custom/sat_icon.png"))); // NOI18N
	...
	{// GEN-HEADEREND:event_removeSatButtonActionPerformed
		...
		if (objectTree.getSelectionCount() != 0) // still might be a root node
		{
			...
			{
				// get object
				MutableTreeNode node = (MutableTreeNode) tp
				...
				if (satHash.containsKey(nameSelected)) {
					...
					((DefaultTreeModel) objectTree.getModel())
							.removeNodeFromParent(node);

					parentApp.setStatusMessage("Deleted Satellite: "
							+ nameSelected);
					...
					parentApp.setStatusMessage("Deleted Ground Station: "
							+ nameSelected);
				}
	...
	{// GEN-HEADEREND:event_infoSatButtonActionPerformed
		...
		if (satHash.containsKey(nameSelected)) {
			...
			String windowName = prop.getName().trim(); // set name - trim excess
														// spaces
			...
			// create property Panel:
			GroundStationInformationPanel newPanel = new GroundStationInformationPanel(
					prop);

			String windowName = prop.getStationName().trim(); // set name - trim
																// excess spaces
	...
	public void openCurrentOptions(Object obj) {
		...
		// check if a ground station was selected
			...
			// create create Sat Settings panel
			GroundStationSettingsPanel newPanel = new GroundStationSettingsPanel(
					prop, parentApp);

			String windowName = prop.getStationName().trim() + " - Settings"; // set
																				// name
																				// -
																				// trim
																				// excess
	...
	private void addCustomSatButtonActionPerformed(
			java.awt.event.ActionEvent evt)// GEN-FIRST:event_addCustomSatButtonActionPerformed
	...
	public void addGS2List(GroundStation prop) {
		...
		treeModel.insertNodeInto(newNode, topGSTreeNode,
				topGSTreeNode.getChildCount());

		newNode.setIcon(new ImageIcon(Toolkit.getDefaultToolkit().getImage(
				getClass().getResource("/icons/custom/groundStation_obj.png"))));
	...
	public void refreshObjectList() {
		...
		for (int i = 0; i < children; i++) {
			MutableTreeNode child = (MutableTreeNode) treeModel.getChild(
					topSatTreeNode, 0); // always get 0th element
		...
		for (int i = 0; i < children; i++) {
			MutableTreeNode child = (MutableTreeNode) treeModel.getChild(
					topGSTreeNode, 0); // always get 0th element
			treeModel.removeNodeFromParent(child);
		}
