...
public class JTrackingPanel extends javax.swing.JPanel {
	...
	// in future might not want to search hash each iteration for gs and sat
	// objects
	public void updateTime(String timeAsString) {
		...
		// get sat and gs objects they are null (otherwise same or if boxes
		// changed?)
		// for now local vars = get them each time.
		GroundStation gs = gsHash.get(gsComboBox.getSelectedItem().toString());
		AbstractSatellite sat = satHash.get(satComboBox.getSelectedItem()
				.toString());

		// calculate AER
		// double[] aer = gs.calculate_AER( sat.getJ2000Position() ); // in J2k
		// position - incorrect
		if (sat.getJ2000Position() != null
				&& !nanDbl.equals(sat.getJ2000Position().getX())) // check for
																	// NAN
		{
			double[] aer = gs.calculate_AER(sat.getJ2000Position().toArray()); // MOD
			...
			jPolarPlotLabel.setGs2SatNameString(gs.getStationName().trim()
					+ " to " + sat.getName().trim());
			...
			// 2D and polar plot
			if (jPolarPlotLabel.isShowLeadLagData()
					&& sat.getSatOptions().isShowGroundTrack()) {
				boolean updateLeadData = false;
				boolean updateLagData = false;

				// need to update lead data?
				if (jPolarPlotLabel.getAerLead() == null
						|| jPolarPlotLabel.getAerLead().length < 1) {
					updateLeadData = true;
				...
				if (jPolarPlotLabel.getAerLag() == null
						|| jPolarPlotLabel.getAerLag().length < 1) {
					updateLagData = true;
				...
				if (updateLeadData) // || check to see if lead/lag data has been
									// updated..
				{
					jPolarPlotLabel.setAerLead(AER.calculate_AER(
							new double[] { gs.getLatitude(), gs.getLongitude(),
									gs.getAltitude() }, sat.getTemePosLead(),
							sat.getTimeLead()));
					// System.out.println("Lead updated");
				}

				// update lag data if needed
				if (updateLagData) // || heck to see if lead/lag data has been
									// updated..
				{
					jPolarPlotLabel.setAerLag(AER.calculate_AER(
							new double[] { gs.getLatitude(), gs.getLongitude(),
									gs.getAltitude() }, sat.getTemePosLag(),
							sat.getTimeLag()));
	...
	private void initComponents() {
		...
		gsComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
		...
		satComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
				"Item 1", "Item 2", "Item 3", "Item 4" }));
		...
		refreshComboBoxesButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/view-refresh.png"))); // NOI18N
		refreshComboBoxesButton
				.setToolTipText("refresh sat and ground stations"); // NOI18N
		refreshComboBoxesButton
		...
		jButton1.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"/icons/gnome_2_18/document-print.png"))); // NOI18N
		...
		printTableButton.setIcon(new javax.swing.ImageIcon(getClass()
				.getResource("/icons/gnome_2_18/document-print.png"))); // NOI18N
		...
		saveTableButton.setIcon(new javax.swing.ImageIcon(getClass()
		...
		azComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
				"Degrees", "Compass Points" }));
		...
		layout.setVerticalGroup(layout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addComponent(
				jTabbedPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 321,
				Short.MAX_VALUE));
	...
	private void objectChangeReset() {
		...
		if (passTableModel != null) // ini of object
		{
			...
			GroundStation gs = gsHash.get(gsComboBox.getSelectedItem()
					.toString());
			AbstractSatellite sat = satHash.get(satComboBox.getSelectedItem()
					.toString());
			if (jPolarPlotLabel.isShowLeadLagData()
	...
	public void runPassPrediction() throws OrekitException {
		...
		// // AER.calculate_AER(new double[]{gs.getLatitude(),
		// gs.getLongitude(), gs.getAltitude()}, sat.getModPosLead(),
		// sat.getTimeLead())
		// double time0 = jdStart - timeStepSec/(60.0*60.0*24.0);
		// h0 = AER.calculate_AER(gs.getLla_deg_m(),
		// sat.calculateMODPositionFromUT(time0) , time0)[1];
		// double time1 = jdStart;
		// h1 = AER.calculate_AER(gs.getLla_deg_m(),
		// sat.calculateMODPositionFromUT(time1) , time1)[1];
		// double time2 = jdStart + timeStepSec/(60.0*60.0*24.0);; // declare
		// var
		//
		...
		// for(double jd = jdStart; jd <= jdStart + timeSpanDays; jd +=
		// timeStepSec/(60.0*60.0*24.0))
		// {
		// time0 = time1;
		// time1 = time2;
		...
		// h2 = AER.calculate_AER(gs.getLla_deg_m(),
		// sat.calculateMODPositionFromUT(time2) , time2)[1];
		// System.out.println(time2 + "," + h2 );
		//
		// // create a quadratic interpolator
		// QuadraticInterpolatorSimp qis = new QuadraticInterpolatorSimp(time0,
		// h0, time1, h1, time2, h2);
		//
		...
		Sun internalSun = new Sun(jdStart - AstroConst.JDminusMJD); // new sun
																	// for
																	// internal
																	// calculations
																	// of
		...
		for (double jd = jdStart; jd <= jdStart + timeSpanDays; jd += timeStepSec
			...
			if (h0 <= 0 && h1 > 0) {
				...
				if (azComboBox.getSelectedIndex() == 0) {
					...
					passTableModel.setValueAt(
							CoordinateConversion.degrees2CompassPoints(az),
							passTableModel.getRowCount() - 1, 2);
				}
			}
			...
			if (h1 <= 0 && h0 > 0) {
				...
				if (lastRise > 0) {
					...
					passTableModel.setValueAt(durStr,
							passTableModel.getRowCount() - 1, 5); // last row,
																	// 4rd
																	// column
																	// (3)
				}
				...
				if (lastRise > 0) {
					...
					double twilightOffset = 6; // degrees extra required for
												// darkness

					// set the suns time
					internalSun.setCurrentMJD(julDateVizCalc
							- AstroConst.JDminusMJD);

					// MOD - sun dot site positions to determine if station is
					// in sunlight
					Vector3D gsECI = AER.calculateECIposition(julDateVizCalc,
							gs.getLla_deg_m());
					double sunDotSite = internalSun.getCurrentPositionTEME()
							.dotProduct(gsECI);

					// TEST - find angle between sun -> center of Earth ->
					// Ground Station
					double sinFinalSigmaGS = internalSun
							.getCurrentPositionTEME().crossProduct(gsECI)
							.getNorm()
							/ (internalSun.getCurrentPositionTEME().getNorm() * gsECI
									.getNorm());
					double finalSigmaGS = Math.asin(sinFinalSigmaGS) * 180.0
							/ Math.PI; // in degrees

					if (sunDotSite > 0
							|| (90.0 - finalSigmaGS) < twilightOffset) {
						passTableModel.setValueAt("Radar Sun",
								passTableModel.getRowCount() - 1, 6); // last
																		// row,
																		// 5rd
																		// column
																		// (4)
						...
						double sinFinalSigma = internalSun
								...
								.getNorm()
								/ (internalSun.getCurrentPositionTEME()
										.getNorm() * satMOD.getNorm());
						double finalSigma = Math.asin(sinFinalSigma);
						double dist = satMOD.getNorm()
								* Math.cos(finalSigma - Math.PI / 2.0);

						if (dist > AstroConst.R_Earth_mean) // changed to mean
															// 30/March/2009 SEG
						{
							// sat is in sunlight!
							passTableModel.setValueAt("Visible",
									passTableModel.getRowCount() - 1, 6); // last
																			// row,
																			// 5rd
																			// column
						...
						{
							passTableModel.setValueAt("Radar Night",
									passTableModel.getRowCount() - 1, 6); // last
																			// row,
																			// 5rd
																			// column
	...
	{// GEN-HEADEREND:event_printTableButtonActionPerformed
		...
		try {
			...
			// MessageFormat("JSatTrak Pass Predictions: \n" +
			// gsComboBox.getSelectedItem().toString() + " to " +
			// satComboBox.getSelectedItem().toString());
			MessageFormat header = new MessageFormat(gsComboBox
					.getSelectedItem().toString()
					+ " to "
					+ satComboBox.getSelectedItem().toString());

			boolean complete = passTable.print(PrintMode.FIT_WIDTH, // JTable.PrintMode
																	// or
																	// PrintMode.NORMAL
					header, // MessageFormat headerFormat
					new MessageFormat("JSatTrak Pass Predictions  - {0} -"), // MessageFormat
																				// footerFormat
	...
	public void go2pass(int passNumber) {
		...
		if (passNumber >= 0) {
			// get # in that row
			Integer passNum = new Integer(passTableModel.getValueAt(passNumber,
					0).toString());
