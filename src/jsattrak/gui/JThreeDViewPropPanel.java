...
public class JThreeDViewPropPanel extends javax.swing.JPanel {
	...
	private static final long serialVersionUID = -3764611924110029340L;
	...
	public JThreeDViewPropPanel(JSatTrak app, J3DEarthComponent threeDPanel,
			WorldWindow wwd) {
		this.app = app;
		...
		// //System.out.println("Eye pos: lat: deg:" +
		// ((OrbitView)wwd.getView()).getLatitude().getDegrees());
		// double viewPitch = ((OrbitView)
		// wwd.getView()).getPitch().getDegrees();
		// double viewHeading = ((OrbitView)
		// wwd.getView()).getHeading().getDegrees();
		// System.out.println("pitch:" + viewPitch + ", heading:" +
		// viewHeading);
		//
		// //double viewLat = ((OrbitView)
		// wwd.getView()).getLatitude().getDegrees();
		// double viewLat = ((OrbitView)
		// wwd.getView()).getLookAtLatitude().getDegrees();
		// double viewLon = ((OrbitView)
		// wwd.getView()).getLookAtLongitude().getDegrees();
		// double viewAlt = ((OrbitView) wwd.getView()).getAltitude();
		// System.out.println("lat:" + viewLat + "(" + ((OrbitView)
		// wwd.getView()).getLatitude().getDegrees() + ")"+", lon:" + viewLon +
		// ", alt:"+viewAlt);
		...
		if (threeDPanel.isModelViewMode()) {
			...
			modeViewComboBox.setSelectedItem(threeDPanel.getModelViewString()); // hope
																				// this
																				// works?
		} else {
		...
		terminatorColorLabel.setBackground(threeDPanel
				.getEcefTimeDepRenderableLayer().getTerminator().getColor());
	...
	private void initComponents() {
		...
		jPanel1.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Camera Properties"));
		...
		modeViewComboBox.setModel(new javax.swing.DefaultComboBoxModel(
		...
		jPanel3.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Lighting Effects / Sun"));
		...
		terminatorColorLabel.setBorder(javax.swing.BorderFactory
				.createLineBorder(new java.awt.Color(0, 0, 0)));
		terminatorColorLabel.setOpaque(true);
		terminatorColorLabel
		...
		jPanel5.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Clipping Planes"));
	...
	private void earthViewRadioButtonStateChanged(
			javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_earthViewRadioButtonStateChanged
	...
	private void modelViewRadioButtonStateChanged(
			javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_modelViewRadioButtonStateChanged

		if (modelViewRadioButton.isSelected()) {
			modeViewComboBox.setEnabled(modelViewRadioButton.isSelected());
			ecefRadioButton.doClick();

			eciRadioButton.setEnabled(false); // hide so user can't seletct this
												// (not possible in this view
												// anyways)
		}
	...
	{// GEN-HEADEREND:event_gridColorLabelMouseClicked
		// color selector
		Color newColor = JColorChooser.showDialog(this,
				"Choose ECI Grid Line Color", gridColorLabel.getBackground());
	...
	private boolean saveSettings() {
		...
		if (modelViewRadioButton.isSelected()) {
			threeDPanel.setModelViewString(modeViewComboBox.getSelectedItem()
					.toString()); // save name
		...
		threeDPanel.setAmbientLightLevel(ambientSlider.getValue()); // causes
																	// re-render
																	// always

		threeDPanel.getEcefTimeDepRenderableLayer().setShowTerminatorLine(
				terminatorCheckBox.isSelected());

		threeDPanel.getEcefTimeDepRenderableLayer().getTerminator()
				.setColor(terminatorColorLabel.getBackground());

		// grid
		threeDPanel.getEciRadialGrid()
				.setShowGrid(eciGridCheckBox.isSelected());
		threeDPanel.getEciRadialGrid().setColor(gridColorLabel.getBackground());

		// Clipping planes
		try {
			threeDPanel.setOrbitNearClipDistance(Double
					.parseDouble(orbitNearTextField.getText().trim()));
			threeDPanel.setOrbitFarClipDistance(Double
					.parseDouble(orbitFarTextField.getText().trim()));
			threeDPanel.setModelViewNearClip(Double
					.parseDouble(modelNearTextField.getText().trim()));
			threeDPanel.setModelViewFarClip(Double
					.parseDouble(modelFarTextField.getText().trim()));
		} catch (Exception e) {
			System.out.println("Error in reading user input clipping values: "
					+ e.toString());
		}
