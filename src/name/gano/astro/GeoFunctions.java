...
public class GeoFunctions {
	 ...
	 * 
	 * @param ptPos
	 *            Mean of date position vector
	 * @param mjd
	 *            modified julian date (is this UTC or TT?) guessing UTC
	 * @return vector of geodetic [latitude,longitude,altitude]
	 */
	public static double[] GeodeticLLA(Vector3D ptPos, double mjd) {
		...
		return calculateGeodeticLLA(ptPos, daysSinceY2k);
	} // GeodeticLLA
	...
	private static double[] calculateGeodeticLLA(Vector3D ptPos, double d) {
		...
		final double X = ptPos.getX(); // Cartesian coordinates
		final double Y = ptPos.getY();
		final double Z = ptPos.getZ();
		...
		// LLA[1] = LLA[1] -(280.4606 +360.9856473*d)*Math.PI/180.0; // shift
		// based on time
		// add fidelity to the line above
		LLA[1] = LLA[1] - earthRotationDeg(d) * Math.PI / 180.0; // shift based
																	// on time
	...
	private static double earthRotationDeg(double d) // days since y2K
	{
		...
		// do calculation
		return ((280.46061837 + 360.98564736629 * (d)) + 0.000387933 * T * T - T
	...
	public static double[] lla2ecef(double lambda, double phi, double alt) {
		...
		double e_sqr = AstroConst.f_Earth * (2.0 - AstroConst.f_Earth); // Square
																		// of
																		// eccentricity
		...
		// Variables
		//
		double N = AstroConst.R_Earth_major
				/ Math.sqrt(1.0 - e_sqr * (sin_phi * sin_phi));
	...
	public static double[] ecef2lla_Fast(double[] pos) // d is current MDT time
	{
		...
		double a = 6378137;
		double e = 8.1819190842622e-2; // 0;%8.1819190842622e-2/a;%8.1819190842622e-2;
										// % 0.003352810664747

		double b = Math.sqrt(Math.pow(a, 2.0) * (1 - Math.pow(e, 2)));
		double ep = Math.sqrt((Math.pow(a, 2.0) - Math.pow(b, 2.0))
				/ Math.pow(b, 2.0));
		double p = Math.sqrt(Math.pow(pos[0], 2.0) + Math.pow(pos[1], 2.0));
		double th = Math.atan2(a * pos[2], b * p);
		lla[1] = Math.atan2(pos[1], pos[0]);
		lla[0] = Math.atan2(
				(pos[2] + Math.pow(ep, 2.0) * b * Math.pow(Math.sin(th), 3.0)),
				(p - Math.pow(e, 2.0) * a * Math.pow(Math.cos(th), 3.0)));
		double N = a
				/ Math.sqrt(1 - Math.pow(e, 2.0)
						* Math.pow(Math.sin(lla[0]), 2.0));
		...
		/*
		 * // now correct for time shift // account for earth rotations lla[1] =
		 * lla[1]-(280.4606 +360.9856473*d)*Math.PI/180.0;
		 * 
		 * // correction ?? //lla[1] = lla[1]-Math.PI/2.0;
		 * 
		 * // now insure [-180,180] range double div =
		 * Math.floor(lla[1]/(2*Math.PI)); lla[1] = lla[1] - div*2*Math.PI;
		 * if(lla[1] > Math.PI) { lla[1] = lla[1]- 2.0*Math.PI; }
		 */
	 ...
	 *            Julian Date for AER calculation (corresponds to ECI position)
	 * @param lla_deg_m
	 *            lat long and alt of station in deg/deg/meters (Geodetic)
	 * @param eci_pos
	 *            ECI position of object in meters (sat)
	 * @return Azumuth [deg], Elevation [deg], and Range vector [m]
	 */
	public static double[] calculate_AER(double currentJulianDate,
			double[] lla_deg_m, double[] eci_pos) {
		...
		double thetaDeg = Sidereal.Mean_Sidereal_Deg(currentJulianDate
		...
		// Lat, not geocentric)
		double[] rSEZ = eci2sez(rECI, thetaDeg, lla_deg_m[0]); // ECI vec,
																// sidereal in
																// Deg, latitude
																// in deg
