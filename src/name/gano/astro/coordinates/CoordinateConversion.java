...
public class CoordinateConversion {
	...
	/**
	 * Coverts a vector in J2000.0 coordinates to TEME (true equator, mean
	 ...
	 * 
	 * @param mjd
	 *            modified julian date of the desired coordinate transformation
	 * @param vecJ2k
	 * @return teme vector
	 */
	public static Vector3D J2000toTEME(double mjd, double[] vecJ2k) {
		...
		double[][] A = J2kCoordinateConversion.teme_j2k(
				J2kCoordinateConversion.Direction.from, ttt, 24, 2, 'a'); // 24
																			// =
																			// order(about
																			// what
																			// STK
																			...
																			// matrix
		// rotate vector
		return new Vector3D(J2kCoordinateConversion.matvecmult(A, vecJ2k));
	}
	 ...
	 * @param modVec2
	 */
	public static void j2000toMOD(double mjd, double[] j2kVec1,
			double[] modVec1, double[] j2kVec2, double[] modVec2) {
		double ttt = (mjd - AstroConst.MJD_J2000) / 36525.0;
		double[][] A = J2kCoordinateConversion.mod_j2000(
				J2kCoordinateConversion.Direction.from, ttt);

		// carfeful on pass by reference, must copy values back to orginal array
		// not assign a new object to it
		double[] temp;
	 ...
	 * @param todVec2
	 */
	public static void j2000toTOD(double mjd, double[] j2kVec1,
			double[] todVec1, double[] j2kVec2, double[] todVec2) {
		double ttt = (mjd - AstroConst.MJD_J2000) / 36525.0;
		double ddpsi = 0.0;
		double ddeps = 0.0;
		int nutTerms = 40; // 4, 40, 106 popular values
		double[][] A = J2kCoordinateConversion.tod_j2000(
				J2kCoordinateConversion.Direction.from, ttt, ddpsi, ddeps,
				nutTerms);
	 ...
	 * 
	 * @param mjdCurrentEquinox
	 *            Current Modified Julian Date Equinox
	 * @param currentVec
	 *            Current vector in Earth equatorial coordinates using
	 *            mjdCurrentEquinox equinox date (any units of length)
	 * @param mjdNewEquinox
	 *            Modified Julian Date of equinox to transform to
	 * @return vector with equatorial coordinates with equinox of mjdNewEquinox
	 */
	public static double[] EquatorialEquinoxChange(double mjdCurrentEquinox,
			final double[] currentVec, double mjdNewEquinox) {
		// first get precession matrix between the two dates
		double[][] prec = PrecMatrix_Equ(
				(mjdCurrentEquinox - AstroConst.MJD_J2000) / 36525,
				(mjdNewEquinox - AstroConst.MJD_J2000) / 36525);
	 ...
	 * 
	 * @param mjdNewEquinox
	 *            new equinox date (MJD)
	 * @param j2kPos
	 *            current J2000.0 vector
	 * @return transformed coordintes
	 */
	public static double[] EquatorialEquinoxFromJ2K(double mjdNewEquinox,
			final double[] j2kPos) {
		return EquatorialEquinoxChange(AstroConst.MJD_J2000, j2kPos,
				mjdNewEquinox);
	}
	 ...
	 * 
	 * @param mjdCurrentEquinox
	 *            current Modified Julian Date of equinox
	 * @param currentPos
	 *            current vector
	 * @return J2000.0 equatorial position vector
	 */
	public static double[] EquatorialEquinoxToJ2K(double mjdCurrentEquinox,
			final double[] currentPos) {
		return EquatorialEquinoxChange(mjdCurrentEquinox, currentPos,
				AstroConst.MJD_J2000);
	}
	 ...
	 * 
	 * @param T1
	 *            Epoch given (current) in Julian centuries since J2000
	 * @param T2
	 *            Epoch to precess to in Julian centuries since J2000
	...
	public static double[][] PrecMatrix_Equ(double T1, double T2) {
		...
		zeta = ((2306.2181 + (1.39656 - 0.000139 * T1) * T1) + ((0.30188 - 0.000344 * T1) + 0.017998 * dT)
				* dT)
				* dT / AstroConst.Arcs;
		z = zeta + ((0.79280 + 0.000411 * T1) + 0.000205 * dT) * dT * dT
				/ AstroConst.Arcs;
		...
		return MathUtils.mult(
				MathUtils.mult(MathUtils.R_z(-z), MathUtils.R_y(theta)),
				MathUtils.R_z(-zeta));
	}
	 ...
	 * 
	 * @param Mjd_1
	 *            Epoch given (Modified Julian Date TT)
	 * @param MjD_2
	 *            Epoch to precess to (Modified Julian Date TT)
	 * @return Precession transformation matrix
	 */
	public static double[][] PrecMatrix_Equ_Mjd(double Mjd_1, double Mjd_2) {
		...
		zeta = ((2306.2181 + (1.39656 - 0.000139 * T) * T) + ((0.30188 - 0.000344 * T) + 0.017998 * dT)
				* dT)
				* dT / AstroConst.Arcs;
		z = zeta + ((0.79280 + 0.000411 * T) + 0.000205 * dT) * dT * dT
				/ AstroConst.Arcs;
		theta = ((2004.3109 - (0.85330 + 0.000217 * T) * T) - ((0.42665 + 0.000217 * T) + 0.041833 * dT)
				* dT)
				* dT / AstroConst.Arcs;
	...
	public static double[][] NutMatrix(double Mjd_TT) {
		...
		return MathUtils.mult(
				MathUtils.mult(MathUtils.R_x(-eps - deps_dpsi[0]),
						MathUtils.R_z(-deps_dpsi[1])), MathUtils.R_x(+eps));
	...
	public static double MeanObliquity(double Mjd_TT) {
		...
		return AstroConst.Rad
				* (23.43929111 - (46.8150 + (0.00059 - 0.001813 * T) * T) * T
						/ 3600.0);
	}
	...
	public static double[] NutAngles(double Mjd_TT) // returns {dpsi,deps}
	{
		...
		Om = MathUtils.Modulo(450160.280 - (5.0 * rev + 482890.539) * T + 7.455
				* T2 + 0.008 * T3, rev);
		...
		for (int i = 0; i < N_coeff; i++) {
			arg = (C[i][0] * l + C[i][1] * lp + C[i][2] * F + C[i][3] * D + C[i][4]
					* Om)
					/ AstroConst.Arcs;
	...
	public static double GAST(double Mjd_UT1) {
		return MathUtils.Modulo(GMST(Mjd_UT1) + EqnEquinox(Mjd_UT1),
				2.0 * Math.PI);
	}
