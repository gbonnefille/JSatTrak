...
public class WWModel3D_new implements Renderable {
	...
	private double eciRotAngleDeg = 0; // angle satellite already rotated due to
										// ECI rotation (through z-axis)
	...
	protected void draw(DrawContext dc, WWModel3D_new model) {
		...
		if (dc.getView().getFrustumInModelCoordinates().contains(loc)) {
			// MAYBE REPLACE "PUSH REFERENCE CENTER" - with gl. move to new
			// center... (maybe not)
			...
			gl.glScaled(localSize, localSize, localSize); // / can change the
															// scale of the
															// model here!!

			// re/un rotate ECI layer
			// System.out.println("------------");
			gl.glRotated(-eciRotAngleDeg, 0.0, 1.0, 0.0); // -- WORKS!!?!! (re
															// rotate because
															// pushReferenceCenter,
															// undoes the ECI
															// roation?)
			...
			// // Earth fixed Inertial -- yes, inertial? (no, because it was
			// spun around by ECI layer)
			// double radius = 0.6;
			// gl.glLineWidth(3.0f);
			...
			// // plot velocity Vector here -- NEEDS A DEROTATION FROM ECI
			// coordinates
			// // because model was spun by xxx degrees already (from ECI layer)
			// around the z-axis so attitude is not where it normally is
			// gl.glColor3d(0,1,1); // COLOR
			// gl.glBegin(gl.GL_LINES);
			// //gl.glVertex3d(-velUnitVec[0]*radius*3, velUnitVec[2]*radius*3,
			// velUnitVec[1]*radius*3);
			// gl.glVertex3d(-velUnitVec[0]*radius*1, velUnitVec[2]*radius*1,
			// velUnitVec[1]*radius*1);
			// gl.glVertex3d(0, 0, 0);
			// gl.glEnd();

			// test normal compontnet -- only works when angle2Rad == 0 below
			// (used for inital debug
			// gl.glColor3d(1,0,1); // COLOR - magenta
			// gl.glBegin(gl.GL_LINES);
			// gl.glVertex3d(-testNorm[0]*radius*4, testNorm[2]*radius*4,
			// testNorm[1]*radius*4);
			// gl.glVertex3d(0, 0, 0);
			// gl.glEnd();

			// rotate around specified angle to align with velocity
			gl.glRotated(angle, -xAxis, zAxis, yAxis); // in J2K coordinate
														// frame
	...
	public void setMainRotationAngleAxis(double[] v, double[] pos) {
		...
		double[] unitOrient = new double[] { orgOrientation[0] / normOr,
				orgOrientation[1] / normOr, orgOrientation[2] / normOr };
		...
		// JOptionPane.showMessageDialog(this, "Angle: " +
		// (angleRad*180.0/Math.PI) + ",\n normAxis: " + normAxis + ",\n Axis: "
		// + axis[0] +"," + axis[1] +"," + axis[2]);

		// System.out.println("MOD unit VEL:" + unitV[0] +"," + unitV[1] +"," +
		// unitV[2]);
		...
		// compute angle for rotation require to align sat body normal to
		// veleoctiy (up away from Earth)
		...
		double[][] rotThroughAxis = new double[][] {
				{ t * X * X + c, t * X * Y - s * Z, t * X * Z + s * Y },
				{ t * X * Y + s * Z, t * Y * Y + c, t * Y * Z - s * X },
				{ t * X * Z - s * Y, t * Y * Z + s * X, t * Z * Z + c } };
		double[] resultingNormDir = MathUtils.UnitVector(MathUtils.mult(
				rotThroughAxis, normTop));
		...
		double[] prosProjVelPlane = MathUtils.UnitVector(MathUtils.sub(pos,
				MathUtils.scale(unitV, MathUtils.dot(pos, unitV))));

		// double rndDOTppvp = MathUtils.dot(resultingNormDir,
		// prosProjVelPlane);
		// find angle between this and the resultingNormDir
		// method 1 - less accurate (around 1/-1), faster CPU-wise
		angle2Rad = Math
				.acos(MathUtils.dot(resultingNormDir, prosProjVelPlane)); // assumes
																			// smallest
																			// angle,
																			// no
																			// +/-
		...
		// angle2Rad = Math.atan2(
		// MathUtils.norm(MathUtils.cross(resultingNormDir, prosProjVelPlane)) ,
		// MathUtils.dot(resultingNormDir, prosProjVelPlane) );

		// check sign of angle2Rad by doting prosProjVelPlane with the body
		// fixed x-axis
		double[] bodyXUnit = MathUtils.UnitVector(MathUtils.mult(
				rotThroughAxis, new double[] { 1, 0, 0 }));
		...
		// System.out.println("Rot-body: " + (angle2Rad*180/Math.PI) + " " +
		// resultingNormDir[0]+ " " + resultingNormDir[1]+ " " +
		// resultingNormDir[2]+ " " + prosProjVelPlane[0]+ " " +
		// prosProjVelPlane[1]+ " " + prosProjVelPlane[2]);
	...
	// update axis and alll rotational things when time updates -- currently
	// only runs when new 3D model is loaded - otherwise similar functions run
	// from OrbitModelRenderable to reduce CPU expense
	public void updateAttitude(AbstractSatellite sat) {
		...
		setPosition(new Position(Angle.fromRadians(sat.getLatitude()),
				Angle.fromRadians(sat.getLongitude()), sat.getAltitude()));
		...
		double rotateECIdeg = ((280.46061837 + 360.98564736629 * (MJD - 51544.5))
				+ 0.000387933 * T * T - T * T * T / 38710000.0 + ECIRenderableLayer.offsetRotdeg) % 360.0;
